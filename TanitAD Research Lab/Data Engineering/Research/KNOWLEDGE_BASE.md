...
- [2026-09-02] [MEASURED ours, banked v2ep episodes + 4 banked arm `config.json`] **THE EPISODE CACHE IS ~199 FRAMES, NOT THE 120 THE TRAINER'S REACHABILITY DOCSTRING ASSUMES, AND THE STRATEGIC BAND IS REACHABLE TO ~18 s NOT 10** - the banked `o4_n` window counts settle it exactly (415,002 at `max_horizon` 20 vs 319,002 at 60; the difference is **exactly 40 x 2,400 episodes**, so both give mean T = 198.92); at T=120 the same arithmetic predicts 225,600 vs the banked 415,002, **off by 1.84x**. `reachable_strategic_ticks`'s docstring says K=6 is where "the corpus is exhausted" - MEASURED it yields **75.17 windows/episode, 0/24 episodes dropped**.
  impact: **the corpus is NOT the binding constraint on the strategic band below ~18 s (compute and the one-tick collapse are), and K=6 (12 s) / K=7 (14 s) straddle MM-E15's 12.5 s median manoeuvre start; the CODE is correct - only the docstring's worked table is wrong** - `TanitAD Research Lab/Data Engineering/Research/2026-09-02-future-action-horizon-and-window-budget/RESULT.md`
- [2026-09-02] [MEASURED ours, action autocorrelation over 24 banked episodes] **THE O11 TIME-SHIFTED NEGATIVE IS AFFORDABLE AND THAT IS THE WRONG QUESTION - AT THE LARGEST FREE SHIFT THE "COUNTERFACTUAL" STEERING IS STILL r = 0.744 WITH THE TRUE ACTION** - the available future-action horizon EQUALS `max_horizon` exactly (`_contract.py:135`), so the queued O11 re-run (`o5_k` 8, `max_horizon` 20, `o11_k` 4) has a free shift budget of s <= 16 - but steer decorrelates only at s ~ 26 (|r| 0.50) and s ~ 41 (|r| 0.25).
  impact: **a negative that is mostly the positive pulls InfoNCE toward its floor and READS AS ACTION-BLINDNESS THAT IS NOT THERE - the design target is `max_horizon >= o11_k + s*`; recommend `--max-horizon 45` (-14.5% windows), cheaper than the grouped-batch alternative and with no parity question** - same package

- [2026-09-05] [source] `train_v6_staged.py:6297-6329` + 3 banked arm configs — the F-11 corpus-RE-SELECTION census and its refusals live inside `if w_stage.w_s1_multi:`, and `w_s1_multi = 0.0` in 3/3 banked configs (9 readings), so the guard has NEVER EXECUTED on any arm we can inspect; `max_h` is set OUTSIDE the branch, so the drop condition is live for every arm while only the test is conditional — impact: hoisting it also prints `shortest_episode_frames` on every run for free, which is what P-18 wanted a pod job for — link: `Data Engineering/Research/2026-09-05-f11-census-guard-scope/RESULT.md`

[2026-09-09] [lib 2606.28383, FULL TEXT] A zero-label clip-value score is just the JEPA's own prediction error ||z_hat-z||_2 (AP 0.512 vs 0.436 chance), but its CONSTANT-VELOCITY baseline reads rho=0.314 - impact: H-DATA-1/row 22 is re-scoped to score-and-floor first (a clip-value number without a kinematic floor is uninterpretable), and backlog row 4's "fixed tau=0.996 has no published operating point" is RETRACTED (fixed alpha=0.996, no-EMA = 44x collapse) - TanitAD Research Lab/Data Engineering/Research/2026-09-09-control-relevant-clip-value/RESULT.md
- [2026-09-10] [arXiv 2606.16533 Kairos, v1 AND v3 full text, fifth route] The two-level control-relevant curation we have carried as RELAYED since 09-02 is a **PROPOSAL** — verbatim *"The current pipeline does not yet compute CID directly"* — and it exists **only in v3**; v1 (90 pp.) does not contain it at all — impact: **debt D-8 discharged; the claim is UNSUPPORTED-AS-STATED as evidence and may not decide a GPU-day; its six-family event taxonomy transfers free and TanitAD can run the measurement Kairos deferred (new row DE10-1)**; ⛔ **and every cited arXiv primary now carries a version stamp, because reading v1 would have filed a false REFUTED** — `Data Engineering/Research/2026-09-10-kairos-curation-fourth-route/RESULT.md`
- [2026-09-10] [arXiv 2609.00111v1 Qwen-Drive-1.0, §2.3] A shipped, measured curation pipeline: LLM **rewrite ≠ validate** (a separate consistency filter, **5.53M → 3.09M, 55.9 % retention**), judges answering **classification questions aggregated programmatically rather than scalar scores**, traces **rejected for revealing future information**, rarity-scored rare-scene upsampling, **longitudinal and lateral manoeuvre components kept separate**, and ⭐ **spatial unification performed on PREDICTED FEATURES rather than on labels** (one differentiable trilinear resample onto each source's native grid) — impact: **the mechanism for combining PhysicalAI-AV with ZOD / PandaSet / YouTube without the relabelling distortion, plus a leak guard at generation time and independent corroboration of our LAT/LON separation** — `Opponent Analysis/Research/2026-09-10-qwen-drive-teardown/RESULT.md`
- [2026-09-13] [NuRec 26.04 HF card + MEASURED clip-id overlap] `map.xodr` carries a zero-ego posted limit only for the **1,607 NuRec clips** — **16/4,572 train and 1/147 eval** of ours, sparser than the CoT sign readings (31) — and its values are km/h labelled mph and per-road autolabels — impact: xodr is an AlpaSim eval-time supplier, not a training supplier; the training-supplier question becomes a sign-present-rate census — `Data Engineering/Research/2026-09-13-posted-speed-limit-supplier/RESULT.md`

- [2026-09-17] [MEASURED, E-DE-SIGN-2 + arXiv 2606.02956] **The CoT speed-limit mention set is 69/4,572 on TRAIN and EXACTLY 0/147 on EVAL** — and the zero is real, proved by a same-breath traffic-light control reading **26/147** on the same pass over the same file; of the 69, **66 are Vienna-convention and 33 are night** (69/0/0 country agreement against `data_collection.parquet`) — impact: **`E-DE-SIGN-1` has NO validation reference on the split it will be scored on**, which is a harder blocker than its plan records; and **KITScenes is CLOSED as validation GT** at three probes (no documented speed VALUE field) plus **CC BY-NC 4.0 / 4.58 TB**, while the OSM route stays closed because our traces carry no coordinates — `TanitAD Research Lab/Data Engineering/Research/2026-09-17-kitscenes-map-speed-values/RESULT.md`

- [2026-09-18] [MEASURED `E-DE-SIGN-3`] **The CoT carries VALUED posted limits in 43 train clips (42 Vienna, 1 US), 100 % on legal steps, 0 false positives in a 115-match audit; 0 in eval; the boolean flag misses 2 and over-counts 28; the quoted "31 of 4,572" does not reproduce** — `E-DE-SIGN-1` now has a specified train-side oracle (DE18-1) and must still declare no eval reference — `Data Engineering/Research/2026-09-18-cot-sign-values/RESULT.md`
- [2026-09-19] [nuplan-devkit lane.py; waymo-open-dataset map.proto] A map-sourced, zero-ego posted speed limit exists per lane in nuPlan (`speed_limit_mps`, Optional, NaN-guarded) and WOMD (`LaneCenter.speed_limit_mph`, optional); coverage unmeasured — unblocks the max-speed channel's supplier question pending FS19-2 and a PI ruling — `Frontier Scan/Daily/2026-09-19/RESULT.md` §3
- [2026-09-19] [MEASURED, nuPlan map DBs v1.1] **Posted-limit coverage is bimodal by city**: Las Vegas and Pittsburgh 100 %, Boston 7.1 %, Singapore 0 % (by lane length); values on a 5-mph ladder; `max_speed` is a constant-25 placeholder; ≈ 93 % expected ego-frame coverage (ESTIMATED). PI ruled a non-parity corpus may supply max-speed labels — nuPlan LV+PIT is a full-coverage label source — `TanitAD Research Lab/Data Engineering/Research/2026-09-19-nuplan-posted-limit-coverage/RESULT.md`
- [2026-09-20] [E-DE-SIGN-4 + 2606.08860] The 42-clip sign oracle is frozen (sha256 f9931330..., deterministic), and its committed gate is ONE test not two: it reduces to k >= 34 of 42, the Wilson clause is vacuous at n=42, P(pass | true accuracy 0.80) = 0.5309, and there is NO recall clause -- so the only published reader for this task (95.45 % precision, 53.85 % recall) either fails outright or scores 100 % depending on an unstated abstention rule. — impact: re-specify the gate before it refuses a supplier (LR17-1) — [Data Engineering/Research/2026-09-20-sign-oracle-freeze/RESULT.md]
