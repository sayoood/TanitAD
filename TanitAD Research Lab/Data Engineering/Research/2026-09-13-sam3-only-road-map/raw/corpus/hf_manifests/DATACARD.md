...
⭐ **2026-09-15 — augmentations added (release `aug-2026-09`):** per-frame SAM3 semantic maps (in progress), LiDAR BEV ground truth (315 clips), obstacle tracks and calibration. Everything is additive — see *Augmentations added 2026-09-15* at the end and `AUG_2026_09_MANIFEST.json`.
...
| **LiDAR BEV ground truth** | `lidar_bev_gt/<clip_id>.bevgt.npz` | 315 | complete |
| obstacle tracks (upstream) | `agents/obstacle_offline/` | 4,585 files | complete |
| calibration (upstream) | `calibration/` | 4,719 | complete |
...
* 315 clips (134 `b1eval` + 181 `b1train200`), schema `tanitad.lidar_bev_gt/2`: occupancy, observed and
...
* The published `obstacle.offline` cuboids (source `scene:obstacles:autolabels:v2`), rows unchanged: 4,585 clips with a source file
  (6 of them with zero cuboids) and 134 with no upstream file. 29,415,112 rows. Top classes: automobile 21,914,553, person 5,518,974, rider 650,067, heavy_truck 562,856, trailer 286,079, bus 209,773.
...
* `camera_intrinsics.parquet` (33,033 rows): width, height, cx, cy, f-theta `fw_poly_0..4` / `bw_poly_0..4` for every camera of every corpus clip.
* `sensor_extrinsics.parquet` (62,317 rows): each sensor's pose in the rig frame (quaternion qx qy qz qw + translation, metres).
