...
def main():
    ...
    out = Path(f"/home/nvidia/sam3map/{c8}_v31raw"); out.mkdir(parents=True, exist_ok=True)
    sd = V2 / f"seq_{c8}"
    ...
    for j in idx:
        ...
        np.savez_compressed(out / f"{j:03d}.npz", tok=tok, T_world_rig=T, stats=json.dumps(stats_all), ver="v3.1",
