...
# v5e arms, PRE-REGISTERED 2026-09-13 before their first run. MEASURED (a2_robust.py, robust ground, tall obstacle points on any
# on-road class, night clip, all 7 cameras): nearest v6.1 0.0502 -> majority v6.5s 0.0708 -> v6.5d 0.0742; the hits lie within
# 1 m of the road boundary (a2_attrib.py ROBUST=1: 88.6 %). Cause hypothesis: several FAR 'road' views outvote one NEAR
# sidewalk view, and near views of poles / walls ('no class') never vote. Two levers, one per arm, on top of v6.5d:
#   MAJ_WEIGHT=sharp4   every vote weighted 1 / (1 + (r / 4 m)^2)^2 instead of 1 / (1 + (r / 8 m)^2): near views dominate
#   BG_VOTE_NEAR=10     a 'no class' observation within 10 m votes BACKGROUND with the same weight, with tracked agent boxes
#                       masked out of every camera (they are not seen, so vehicles cannot vote the road away)
# Each arm WORKS on the night clip (all 7) if A2r_onroad <= 0.062 AND fragments <= 90 per 1000 m^2 AND LiDAR curb recall >= 0.75
# AND crosswalk coloured share <= 0.60 (v6.5d: 0.0742 / 69.7 / 0.801 / 0.507; nearest v6.1: 0.0502 / 170.5 / 0.499 / 0.699).
# Anything else is a FAIL, reported as such.
#   RESULTS (MEASURED, night, all 7; pi_checks_73495082f98b_c.json, a2_robust2_73495082f98b_c.log):
#     e1 sharp4   A2r_onroad 0.0645, fragments 84.7, curb recall 0.702, crosswalk 0.492 -> FAIL (A2r, curb recall)
#     e2 bg<=10m  A2r_onroad 0.0194, fragments 105.5, curb recall 0.542, crosswalk 0.530 -> FAIL (fragments, curb recall)
#   e2's extra fragments are background specks (class-0 fragments 123 -> 293) and its lost curbs are road borders where
#   background, not sidewalk, now meets the road -- the boundary-edge rule only drew sidewalk / road borders.
# v5f arm e3, PRE-REGISTERED 2026-09-13 before its first run: e2 plus
#   BG_EDGES=1      edges also where a cell with near background evidence that no labelled surface won (opened 3x3) touches
#                   the opened drivable surface, components >= 10 cells
#   BG_MINCOMP=50   background-won components smaller than 50 cells (0.5 m^2) go back to their labelled majority
# SAME bars as e1 / e2: WORKS on the night clip (all 7) if A2r_onroad <= 0.062 AND fragments <= 90 per 1000 m^2 AND LiDAR curb
# recall >= 0.75 AND crosswalk coloured share <= 0.60. Anything else is a FAIL, reported as such.
#   RESULT e3 (MEASURED, night, all 7; pi_checks_73495082f98b_d.json): A2r_onroad 0.0194, fragments 99.1, curb recall 0.744,
#   crosswalk 0.530 -> FAIL (fragments, curb recall). Fragments by class vs v6.5d: background 123 -> 283, sidewalk 29 -> 90,
#   edges 113 -> 76; the reverted background-won specks were only 1,248 cells, so most specks are cells with no labelled vote.
# v5g arms, PRE-REGISTERED 2026-09-13 before their first run, SAME bars as e1-e3:
#   e4 = e3 + SPECK_FILL=50    after compositing, every connected component of background (0) or sidewalk-verge (7) smaller
#                              than 50 cells (0.5 m^2) whose 1-cell ring is all labelled (1-7) takes its ring's majority
#                              surface (drivable incl. paint -> 1, sidewalk-verge -> 7); edges and paint are never filled over
#   e5 = e4 + BG_EDGE_OPEN=1   the near-background cells that make edges are NOT opened first (3x3 opening removed)
#   RESULTS (MEASURED): night e4 FAIL (curb recall 0.744); night e5 PASS (A2r_onroad 0.0194, fragments 77.9, curb recall 0.759,
#   crosswalk 0.530; costs: edge precision 0.666 -> 0.635, MAP_E 0.522 -> 0.471). DAY e5 against its own pre-registered
#   no-regression bars (v65e5_day.sh): A2r_onroad 0.0084 <= 0.0094, fragments 90.3 <= 105.1, crosswalk 0.536 <= 0.60, but curb
#   recall 0.600 < 0.667 -> FAIL on the day clip (edge precision 0.857 -> 0.821).
# v5h arm e6, PRE-REGISTERED 2026-09-13 before its first run: e5, except that background may VETO THE ROAD ONLY -- a sidewalk-verge
# vote is no longer outvoted by background (BG_VETO=road), so sidewalk / road borders, and their edges, stay where e1-d had them;
# only road cells whose near background weight beats the road weight are removed. WORKS if BOTH clips pass: night (all 7)
# A2r_onroad <= 0.062 AND fragments <= 90 AND curb recall >= 0.75 AND crosswalk share <= 0.60; day (all 7) A2r_onroad <= 0.0094
# AND fragments <= 105.1 AND curb recall >= 0.667 AND crosswalk share <= 0.60. Anything else is a FAIL, reported as such.
#   RESULT e6 (MEASURED): night PASS (A2r_onroad 0.0237, fragments 79.0, curb 0.779, crosswalk 0.530); day FAIL on curb recall
#   (0.626 < 0.667; A2r_onroad 0.0084, fragments 91.4, crosswalk 0.536) -> e6 FAILS (both clips required).
#   MEASURED on the way (occluder_check.py): the tracked boxes' z lies ~1 m under the LiDAR ground, so agent_occluders' hull
#   (z - h/2 - 0.2 .. z + h + 0.2) masked the asphalt in front of every vehicle in every arm that used it (e2-e6).
# PI 2026-09-13: "you don't need the boxes, scince sam 3 is seperating the roads from the rest very well" -> box-free maps:
#   BG_OCC_BOXES=0   background votes without any box masking (vehicles seen near vote background like anything else)
# Arm e6nb, PRE-REGISTERED 2026-09-13 before its first run: e6 with BG_OCC_BOXES=0 and AGENT_DRIVABLE off. SAME bars as e6, both
# clips required: night A2r_onroad <= 0.062 AND fragments <= 90 AND curb recall >= 0.75 AND crosswalk <= 0.60; day A2r_onroad
# <= 0.0094 AND fragments <= 105.1 AND curb recall >= 0.667 AND crosswalk <= 0.60. Anything else is a FAIL, reported as such.
BG_OCC_BOXES = os.environ.get("BG_OCC_BOXES", "1") == "1"
BG_VETO = os.environ.get("BG_VETO", "all")
SPECK_FILL = int(os.environ.get("SPECK_FILL", "0"))
BG_EDGE_OPEN = int(os.environ.get("BG_EDGE_OPEN", "3"))
BG_EDGES = os.environ.get("BG_EDGES") == "1"
BG_MINCOMP = int(os.environ.get("BG_MINCOMP", "0"))
MAJ_WEIGHT = os.environ.get("MAJ_WEIGHT", "8")
BG_VOTE_NEAR = float(os.environ.get("BG_VOTE_NEAR", "0"))
...
def main():
    ...
    for n, f in enumerate(frames):
        ...
        T = Ts[n]
        boxes = np.load(BOX_ROOT / f"seq_{c8}" / toks[n] / "gt.npz")["boxes"] if (COMPOSITE.endswith("_occ") or COMPOSITE.endswith("_occ2") or (BG_VOTE_NEAR > 0 and BG_OCC_BOXES)) else None
        ...
        for cam in cams:
            ...
            if COMPOSITE == "majority_v65":
                wv = ((1.0 / (1.0 + (dist / 4.0) ** 2) ** 2) if MAJ_WEIGHT == "sharp4" else (1.0 / (1.0 + (dist / 8.0) ** 2))).astype(np.float32)
                if BG_VOTE_NEAR > 0:
                    mb = (code == 0) & (dist <= BG_VOTE_NEAR)
                    vbg[flat_i[mb], flat_j[mb]] += wv[mb]
    ...
    if COMPOSITE == "majority_v65":
        ...
        drv = (dw > 0) & (dw >= sw) & (dw >= vbg)
        wlk = (sw > 0) & (sw > dw) & ((sw >= vbg) if BG_VETO == "all" else True)
        stats["cells_background_won"] = int(((dw + sw) > 0).sum() - (drv | wlk).sum())
        if BG_MINCOMP > 0:
            bgw = ((dw + sw) > 0) & ~drv & ~wlk
            n_b, lab_b, st_b, _ = cv2.connectedComponentsWithStats(bgw.astype(np.uint8), connectivity=8)
            small_b = np.isin(lab_b, np.flatnonzero(st_b[:, cv2.CC_STAT_AREA] < BG_MINCOMP)[1:]) if n_b > 1 else np.zeros_like(bgw)
            drv = drv | (small_b & (dw >= sw)); wlk = wlk | (small_b & (sw > dw))
            stats["cells_background_small_reverted"] = int(small_b.sum())
        if SURFACE_MODE >= 3:
        ...
        if BG_EDGES:
            bg_near = seen & (vbg > 0) & ~drv & ~wlk
            bg_c = (cv2.morphologyEx(bg_near.astype(np.uint8), cv2.MORPH_OPEN, np.ones((BG_EDGE_OPEN, BG_EDGE_OPEN), np.uint8)) > 0) if BG_EDGE_OPEN > 1 else bg_near
            e_bg = bg_c & touch
            n_g, lab_g, st_g, _ = cv2.connectedComponentsWithStats(e_bg.astype(np.uint8), connectivity=8)
            e_bg = np.isin(lab_g, np.flatnonzero(st_g[:, cv2.CC_STAT_AREA] >= 10)[1:]) if n_g > 1 else e_bg
            e_boundary = e_boundary | e_bg
            stats["cells_edge_background_boundary"] = int(e_bg.sum())
        out[e_boundary | e_obs] = 5
        if SPECK_FILL > 0:
            filled = 0
            for k_s in (0, 7):
                m_s = (out == k_s) & seen
                n_s, lab_s, st_s, _ = cv2.connectedComponentsWithStats(m_s.astype(np.uint8), connectivity=8)
                for c_s in np.flatnonzero(st_s[:, cv2.CC_STAT_AREA] < SPECK_FILL)[1:]:
                    x_, y_, w_, h_ = st_s[c_s, 0], st_s[c_s, 1], st_s[c_s, 2], st_s[c_s, 3]
                    i0, i1, j0, j1 = max(y_ - 1, 0), min(y_ + h_ + 1, WW), max(x_ - 1, 0), min(x_ + w_ + 1, WH)
                    comp = lab_s[i0:i1, j0:j1] == c_s
                    ring = (cv2.dilate(comp.astype(np.uint8), np.ones((3, 3), np.uint8)) > 0) & ~comp
                    rv = out[i0:i1, j0:j1][ring]
                    if len(rv) == 0 or np.any((rv == 0) | (rv == 255)) and k_s == 7 or np.any(rv == 255):
                        continue
                    if k_s == 0 and np.any(rv == 0):
                        continue
                    n_drv = int(np.isin(rv, (1, 2, 3, 4, 6)).sum()); n_wlk = int((rv == 7).sum())
                    if n_drv + n_wlk == 0:
                        continue
                    sub = out[i0:i1, j0:j1]; sub[comp] = 1 if n_drv >= n_wlk else 7
                    filled += int(comp.sum())
            stats["cells_speck_filled"] = filled
