...
A2r_onroad counts hits on ANY on-road raster class (drivable, line, crosswalk -- mapq_core.ON_ROAD) instead of drivable only,
so a map that turns crosswalk-area cells back into drivable is not charged for obstacles A2 simply stopped seeing.
...
acc = {l: {"A2_hits": 0, "A2_n": 0, "surface": 0, "near_agent": 0, "A2r_hits": 0, "A2r_n": 0, "A2r_onroad_hits": 0, "cells": {}} for l in W}
for f in sorted(npz_dir.glob("[0-9][0-9][0-9].npz")):
    ...
    for l, wm in W.items():
        ...
        a["A2r_hits"] += int(on2.sum()); a["A2r_n"] += len(c2); a["A2r_onroad_hits"] += int(np.isin(c2, mc.ON_ROAD).sum())
...
for l, a in acc.items():
    ...
    rep[l] = {"A2": round(a["A2_hits"] / max(a["A2_n"], 1), 4), "A2_n": a["A2_n"],
              ...
              "A2r_onroad": round(a["A2r_onroad_hits"] / max(a["A2r_n"], 1), 4),
              "A2r_hit_cells_persistent_ge3_frames": round(float((v >= 3).mean()), 3) if len(v) else None}
    print(l, rep[l], flush=True)
Path(f"/home/nvidia/sam3map/a2_robust_{c8}_{len(W)}v.json").write_text(json.dumps(rep, indent=1), encoding="utf-8")
print("ZZA2R-DONEZZ")
