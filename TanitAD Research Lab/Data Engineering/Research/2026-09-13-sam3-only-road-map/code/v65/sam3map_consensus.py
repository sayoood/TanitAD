...
DRIVE = (1, 2, 3, 4, 6)
NO_PROMOTE = os.environ.get("CONSENSUS_NO_PROMOTE") == "1"   # stripe crosswalks: never turn the road BETWEEN stripes into area paint
    ...
    for n, (f, fpath) in enumerate(zip(frames, files)):
        ...
        for cam in cams:
            ...
            if len(vv):
                ...
                new = old.copy()
                area_px = np.isin(old, (2, 3, 6)); strong1 = (old == 1) & (sh >= STRONG_SHARE) & (not NO_PROMOTE)
                if NO_PROMOTE and f"xstripe_{cam}" in f:                       # stripe crosswalks: only stripe pixels become crosswalk
                    xs_ = f[f"xstripe_{cam}"][vv, uu]
                    new[(dc == 3) & area_px & xs_] = 3
                    new[(dc == 3) & (old == 6) & ~xs_] = 1
                else:
