...
the camera images (gt_reproject.py: ray -> smooth LiDAR ground -> world cell). The colour is the BEV ground truth.
v2 (PI 2026-09-13: "why are there uncolored areas near to the objects?" / "you don't need the boxes, since sam 3 is separating the
roads from the rest very well"): WHERE to paint comes from SAM3 alone -- a pixel is painted when that frame's own SAM3 raster of
that camera labels it a road-level surface (drivable, paint, crosswalk, edge, sidewalk-verge), dilated by 4 working pixels so thin
curb edges at a mask border stay visible. No tracked boxes, no LiDAR occluders. The ego body is the extraction's full ego mask
(its small interior patch is an object on the car: painting over it drew coloured blocks on the hood at night).
...
def sam3_ground(cls_small, hw):
    """bool [h*w]: the frame's SAM3 raster (540 x 960) says road-level surface (classes 1-7), dilated 4 working pixels."""
...
for n, (ff, af) in enumerate(zip(ffiles, afiles)):
    ...
    d.text((10, 38), "colour = the BEV ground-truth class under the pixel's ground point · painted only where SAM3 sees a road-level surface in this frame (no boxes)",
    ...
    wxy, keep, hw = GR.camera_ground(C, sg, T, 2, lidar=None, ego_small=za["ego_CAM_FW"] if "ego_CAM_FW" in za.files else None)
    keep &= sam3_ground(za["cls_CAM_FW"], hw)
    ...
    for q, cam in enumerate(SIDE):
        ...
        w2, k2, h2 = GR.camera_ground(Cs, sg, T, 4, lidar=None, ego_small=za[f"ego_{cam}"] if f"ego_{cam}" in za.files else None)
        k2 &= sam3_ground(za[f"cls_{cam}"], h2)
    ...
    d.text((10, 1040), "Ground truth uses future frames and LiDAR ground height" + (" and tracked vehicle boxes" if os.environ.get("FOOTER_BOXES") == "1" else "") + " (label time only); inference is vision-only.",
