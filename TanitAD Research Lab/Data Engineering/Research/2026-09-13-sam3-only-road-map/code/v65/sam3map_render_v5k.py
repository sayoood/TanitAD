...
# v5j arm d1, PRE-REGISTERED 2026-09-14 before its first run, on top of the delivered box-free map d0 (v5d, AGENT_DRIVABLE off).
# MEASURED on d0: a vehicle that stays in view (a van following the ego at the day clip's start) leaves the lane under it
# 'seen, no class' -- a grey strip road-wide on both sides in the BEV; box-free, nothing fills it.
#   OCC_FILL_M=2.5   seen-no-class cells inside the closing of the drivable surface (incl. paint) with a 2.5 m disc become drivable:
#                    strips narrower than 2.5 m with road on both sides fill; a parked car between road and sidewalk (road on one
#                    side only) does not; edges and sidewalk cells are never overwritten.
# d1 WORKS if, on BOTH clips (all 7 cameras): MAP_C >= d0 + 0.03 (night 0.5031, day 0.776) AND A2r_onroad <= d0 + 0.005 (night
# 0.0723, day 0.0093) AND LiDAR curb recall >= d0 - 0.02 (night 0.814, day 0.800) AND fragments <= d0 (night 67.0, day 94.6).
# Anything else is a FAIL, reported as such.
#   RESULT d1 (MEASURED): night MAP_C 0.5031 (bar 0.5331) FAIL, A2r_onroad 0.0725, curb 0.814, fragments 66.4; day MAP_C 0.8929,
#   A2r_onroad 0.0094, curb 0.800, fragments 94.0 (all pass) -> d1 FAILS (night vehicles on road unchanged: parked cars stand
#   between road and sidewalk, road on one side only, which the closing does not fill by design).
# v5k arm d2, PRE-REGISTERED 2026-09-14 before its first run: d1's narrow-strip fill AND a parking-lane fill, both applied BEFORE the
# edges are drawn (so the kerb behind a filled strip gets its edge):
#   PARK_FILL_M=2.5  seen-no-class cells within 2.5 m of the drivable surface AND within 2.5 m of sidewalk-verge become drivable
# SAME bars as d1, both clips: MAP_C >= d0 + 0.03 (night 0.5031, day 0.776) AND A2r_onroad <= d0 + 0.005 (night 0.0723, day 0.0093)
# AND curb recall >= d0 - 0.02 (night 0.814, day 0.800) AND fragments <= d0 (night 67.0, day 94.6). Anything else is a FAIL.
PARK_FILL_M = float(os.environ.get("PARK_FILL_M", "0"))
OCC_FILL_M = float(os.environ.get("OCC_FILL_M", "0"))
...
def main():
    ...
    if COMPOSITE == "majority_v65":
        ...
        out[pm] = paint_k[pm]
        if OCC_FILL_M > 0 or PARK_FILL_M > 0:
            filled_total = 0
            if OCC_FILL_M > 0:
                k_f = int(round(OCC_FILL_M / RES)) | 1
                closed = cv2.morphologyEx(np.isin(out, (1, 2, 3, 4, 6)).astype(np.uint8), cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k_f, k_f))) > 0
                f1 = closed & (out == 0) & seen
                out[f1] = 1; drv = drv | f1; filled_total += int(f1.sum()); stats["cells_occlusion_filled"] = int(f1.sum())
            if PARK_FILL_M > 0:
                d_drv = cv2.distanceTransform((~np.isin(out, (1, 2, 3, 4, 6))).astype(np.uint8), cv2.DIST_L2, 5) * RES
                d_wlk = cv2.distanceTransform((out != 7).astype(np.uint8), cv2.DIST_L2, 5) * RES
                f2 = (out == 0) & seen & (d_drv <= PARK_FILL_M) & (d_wlk <= PARK_FILL_M)
                out[f2] = 1; drv = drv | f2; filled_total += int(f2.sum()); stats["cells_parking_filled"] = int(f2.sum())
        ...
        if False:
            k_f = int(round(OCC_FILL_M / RES)) | 1
            drv_all = np.isin(out, (1, 2, 3, 4, 6)).astype(np.uint8)
            closed = cv2.morphologyEx(drv_all, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k_f, k_f))) > 0
            fill = closed & (out == 0) & seen
            out[fill] = 1
            stats["cells_occlusion_filled"] = int(fill.sum())
