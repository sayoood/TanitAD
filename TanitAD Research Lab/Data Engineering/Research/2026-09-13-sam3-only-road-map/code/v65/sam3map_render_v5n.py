...
# v5m (2026-09-14, PI on the box-free videos: "why is the Sperrflaeche marked as red? why is the separation between yellow and red
# so noisy? it was better in the past. The stripes of the crosswalk are also not well enough separated"): the renderer of the
# delivered map d0 plus FIELDS=1, which saves the per-cell vote fields (worldmap_fields.npz) so composition rules can be tested
# offline on the exact same observations: class weights after paint trimming (vk1..vk7), UNtrimmed paint-class weights
# (vraw2/3/4/6), edge and near weights, background and all-observation weights, and the image evidence for paint: the weighted
# mean of each layer's top-hat divided by that layer's paint threshold (vth) and the weighted share of bright observations (vpf).
# The composition itself is unchanged: worldmap.npz must equal d0's cell for cell (checked outside).
FIELDS = os.environ.get("FIELDS") == "1"
# v5n (2026-09-14): NEAR_PAINT_M=r -- only observations within r m of their camera vote for a PAINT class (line, crosswalk, arrow, hatched);
# a farther paint observation votes plain drivable. MEASURED motivation (xwalk_reproj_contrast.py): the delivered stripe cells sit on the
# paint (bar/gap image top-hat 1.54 night, 2.41 day) but cover 4-7 % of the crossings, and every rule that widened them from the
# multi-view fields (periodic fits, image-evidence stripes) left the paint; far views resolve stripes worst.
NEAR_PAINT_M = float(os.environ.get("NEAR_PAINT_M", "0"))
...
def layer_fields(C, sg, img, cls_small, ego_small, ev_small=None, occ_small=None):
    ...
    if not len(idx):
        return code.reshape(NX, NY), ev.reshape(NX, NY), paint.reshape(NX, NY), {"thr": None, "th": np.zeros((NX, NY), np.float32)}
    ...
    thg = np.zeros(N, np.float32); thg[idx] = th / thr
    return code.reshape(NX, NY), ev.reshape(NX, NY), paint.reshape(NX, NY), {"thr": round(thr, 1), "th": thg.reshape(NX, NY)}
...
def build_layer(C, sg, img, cls_small, ego_small, occ_small=None):
    ...
    return code, {"thin_cells": int(thin.sum()), "trimmed": int(trim.sum()), "thr": st["thr"], "th_ratio": st["th"], "code_raw": raw}
...
def main():
    ...
    if COMPOSITE == "majority_v65":
        ...
        if FIELDS:
            vraw = {k_: np.zeros((WW, WH), np.float32) for k_ in (2, 3, 4, 6)}
            vbgf = np.zeros((WW, WH), np.float32); vobs = np.zeros((WW, WH), np.float32); vth = np.zeros((WW, WH), np.float32); vpf = np.zeros((WW, WH), np.float32)
    ...
    for n, f in enumerate(frames):
        ...
        for cam in cams:
            ...
            if COMPOSITE == "majority_v65":
                ...
                far_paint = np.isin(code, (2, 3, 4, 6)) & (dist > NEAR_PAINT_M) if NEAR_PAINT_M > 0 else np.zeros(code.shape, bool)
                for k_ in (1, 2, 3, 4, 6, 7):
                    mk = (code == k_) & ~far_paint if k_ != 1 else ((code == 1) | far_paint)
                    if mk.any():
                ...
                if FIELDS:
                    craw = st["code_raw"][li, lj]; ratio = st["th_ratio"][li, lj]
                    for k_ in (2, 3, 4, 6):
                        mr = craw == k_
                        if mr.any():
                            vraw[k_][flat_i[mr], flat_j[mr]] += wv[mr]
                    ob = code != 255
                    vobs[flat_i[ob], flat_j[ob]] += wv[ob]
                    mb = code == 0
                    vbgf[flat_i[mb], flat_j[mb]] += wv[mb]
                    vth[flat_i[ob], flat_j[ob]] += wv[ob] * np.clip(ratio[ob], 0, 5)
                    vpf[flat_i[ob], flat_j[ob]] += wv[ob] * (ratio[ob] > 1.0)
    ...
    if COMPOSITE == "majority_v65" and FIELDS:
        f16 = lambda a: a.astype(np.float32)                                  # float32: offline composition must reproduce ties exactly
        np.savez_compressed(out_dir / "worldmap_fields.npz", vk1=f16(vk[1]), vk2=f16(vk[2]), vk3=f16(vk[3]), vk4=f16(vk[4]), vk6=f16(vk[6]), vk7=f16(vk[7]),
                            vraw2=f16(vraw[2]), vraw3=f16(vraw[3]), vraw4=f16(vraw[4]), vraw6=f16(vraw[6]), vedge=f16(vedge), vnear=f16(vnear),
                            vbg=f16(vbgf), vobs=f16(vobs), vth=f16(vth), vpf=f16(vpf), seen=(wcls != 255) if not np.issubdtype(wcls.dtype, np.bool_) else wcls,
                            origin=np.array([wx0, wy0]), res=RES)
