...
def camera_coverage(fd, grid, fb):
    ...
    c = np.load(fd / "calib.npz"); fr = json.loads((fd / "frame.json").read_text(encoding="utf-8"))
    ...
    Pg = np.c_[x, y, z]; seen = np.zeros(len(Pg), bool)
    for cam in VIEWS:
...
def main():
    ...
    out = npz_dir.parent / f"sam3map_score_{npz_dir.name}.json"             # v1 and v2 directories score separately
