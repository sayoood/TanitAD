"""SAM3-ONLY road map, v6 = v5 on ALL NATIVE cameras (CAM_FW, CAM_CL, CAM_CR, CAM_RL, CAM_RR, CAM_RT, CAM_FT) plus
...
def classify(proc, img, lift_ctx, front):
    ...
    for p, s, m in inst["road"]:
        road |= m
    nondrive = np.zeros((H, W), bool); curb = np.zeros((H, W), bool); walk = np.zeros((H, W), bool)
    ...
    for p, s, m in ok["diag"]:
        if front and s >= 0.45 and not (m & (cross | hatch)).any():
...
def main():
    ...
    out = Path(f"/home/nvidia/sam3map/{c8}_v6raw"); out.mkdir(parents=True, exist_ok=True)
    sd = V2 / f"seq_{c8}"
    ...
    for j in idx:
        ...
        np.savez_compressed(out / f"{j:03d}.npz", tok=tok, T_world_rig=T, stats=json.dumps(stats_all), ver="v6",
        ...
        tot = {k: sum(len(a) for a in lifted[k]) for k in range(1, 8)}
