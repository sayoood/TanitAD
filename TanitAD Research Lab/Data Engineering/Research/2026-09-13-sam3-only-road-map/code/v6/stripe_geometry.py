...
def decide2(stripe_union_mask, region_mask, K, R, t, grid, fb):
    """Pinhole + per-cell ground wrapper (v3.1). See decide_points."""
    return decide_points(P.lift(stripe_union_mask, K, R, t, grid, fb, stride=2), P.lift(region_mask, K, R, t, grid, fb, stride=2))


def decide_points(xy, rxy):
    """Mode of local stripe directions vs the region's long axis, both on the ground (points already lifted by ANY
    camera model). zebra: dominant mode >= 60 deg to the region axis; hatched: 20-55 deg, or two strong modes >= 40 deg
    apart (chevron); region elongation >= 1.5 required; everything else ambiguous."""
