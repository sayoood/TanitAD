...
def main(c8s, out):
    ...
    k = 0
    tag = os.environ.get("SAM3MAP_LONG_TAG", "v3"); root = Path(os.environ.get("SAM3MAP_ROOT", str(SM / "front")))
    for q, c8 in enumerate(c8s):
        frames = sorted((SM / f"render_{c8}_{tag}").glob("f*.png"))
        if not frames:
            print("skip (no frames)", c8); continue
        meta = json.loads(next((root / f"seq_{c8}").glob("*/meta.json")).read_text())
        ...
        d.text((120, 590), f"front camera only · ground from the ego path · SAM3-only map {tag}", fill=(150, 158, 154), font=font(26))
        for _ in range(10):
