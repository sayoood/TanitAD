...
CAM = os.environ.get("SAM3MAP_CAM", "CAM_FW")                                 # CAM_F0 for the front_native sequences
...
def frame_inputs(c8, j, toks, sd, poses):
    ...
    camm = E.CM.Camera.from_calib(c, fr["cam_order"].index(CAM), img.width, img.height)
...
def post(fi, cls, st, evid, xs_list, ws_list, dst, j, tag):
    ...
    p_, r_ = relift(cl, E.CM.Camera.from_calib(fi["c"], fi["fr"]["cam_order"].index(CAM)), fi["sgrid"], fi["T"])
    out = {"tok": fi["tok"], "T_world_rig": fi["T"], "ver": "v6f", "stats": json.dumps({CAM: st, "_front_only_from": f"front_fast:{tag}"}),
           f"cls_{CAM}": cl, f"evid_{CAM}": small_e, f"xstripe_{CAM}": xsmall}
    for k in range(1, 8):
