...
def build_sequence(clip):
    ...
    import hashlib
    c8 = clip[:8]; sha = hashlib.sha256(clip.encode()).hexdigest()[:12]; chunk = int(IDX.loc[clip, "chunk"])
...
def cpu_stages(info, ego_path, out_dir):
    ...
    if ok:                                                                     # the clip's working files, only after its checks passed
...
def main():
    ...
    import hashlib
    todo = [c for c in clips if hashlib.sha256(c.encode()).hexdigest()[:12] not in done][:limit]
    T0 = time.time(); proc, fix = FF.E.S.build(conf=0.25); fast = FF.FastSAM3(proc, FF.FLAGS)
    sam3_smoke.build = lambda conf=0.25: (proc, fix)
    print(f"model ready {time.time() - T0:.1f}s; {len(todo)} clips to do, {len(done)} already in the ledger", flush=True)
    ...
    jobs = []
    nxt = pre.submit(build_sequence, todo[0]) if todo else None
    for k, clip in enumerate(todo):
        tc = time.time(); rec = {"clip_sha12": hashlib.sha256(clip.encode()).hexdigest()[:12], "t_start": time.strftime("%Y-%m-%dT%H:%M:%S")}
        try:
            info = nxt.result(); t_seq = time.time() - tc
            nxt = pre.submit(build_sequence, todo[k + 1]) if k + 1 < len(todo) else None
            ...
            jobs.append((rec, bg.submit(cpu_stages, info, ego_path, out)))
        except Exception as e:
            rec.update({"status": "FAIL-GPU-STAGE", "error": f"{type(e).__name__}: {str(e)[:300]}"})
            traceback.print_exc()
            with open(ledger, "a") as fh:
                fh.write(json.dumps(rec) + "\n")
        for r_, j_ in [x for x in jobs if x[1].done()]:
            r_.update(j_.result()); jobs.remove((r_, j_))
            with open(ledger, "a") as fh:
    ...
    for r_, j_ in jobs:
        ...
        with open(ledger, "a") as fh:
            fh.write(json.dumps(r_) + "\n")
    print(f"ZZPROD-DONE {len(todo)} clips {time.time() - T0:.0f}s")
