...
def main(c8):
    ...
    for j in range(len(F)):
        ...
        for cam, (w, cw, h) in cams[j].items():
            r = np.hypot(w[:, 0] - cw[0], w[:, 1] - cw[1]); ref.append(w[r <= 10.0])
    ...
    print(f"== {c8} ground={GROUND}: MARGIN ALL CAMERAS 8-16 m  median signed {np.median(x):+.3f}  median |offset| {np.median(np.abs(x)):.3f}  n {len(x)}")
    for cam in VIEWS:
        line = []
        for b in BINS:
            if acc[cam][b]:
