...
> **Structured along the agents** (PI, 2026-08-18). Each area owns its findings log; this file is
> the router, not a copy. Add findings to the AREA file — nothing is stored here.

> ⛔ **Three layers, and they are not interchangeable.**
> **PAPER** (`../Paper/TANITAD_PAPER.md`) = the scientific account of the frontier work.
> **KNOWLEDGE_BASE** (the area files below) = what we learned, for an agent about to decide.
> **LIBRARY** ([`Library/`](Library/LIBRARY.md)) = the evidence, one banked sha256 per primary.

**175 entries across 7 areas** (Production & Optimization is empty — see its file).
