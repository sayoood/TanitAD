...
cd /root/TanitAD/stack && python3 scripts/v6_chain.py commands \
  ...
  --geometry-from "../TanitAD Research Hub/Architecture & Inference/Implementation/incoming/2026-08-17-st-launch-fixes/raw/v6F-SW-30k.config.json"
```

⛔ **`--geometry-from` IS NOT OPTIONAL OFF-POD, AND THE CHAIN NOW REFUSES WITHOUT IT** (E1, 2026-08-17).
Every line below **carries the ancestor's model geometry**, read out of that run's own
`config.json["args"]` — the only authority on what shapes its checkpoint holds. Before this, the
emitted S-T line carried **no geometry flag at all**, so the stack built at the trainer's defaults:
MEASURED **87 926 473 params / 407 keys / `encoder.pos [1, 640, 384]`** against a **336 575 049-param,
575-key, `[1, 640, 768]`** checkpoint, dying with `RuntimeError … size mismatch for encoder.pos`.
**On the pod** `<root>/v6F-SW-30k/config.json` is found first and wins; **off-pod** the banked copy
above is what makes the line reproducible (md5 `2cb83239cc19d13b9bc7a49a27459b82`, verified identical
to the live run's file on Thor). ⚠️ **Geometry is emitted even where it equals today's default** —
an "omit the defaults" line means whatever the defaults happen to be *on the day it runs*.
...
⛔ **`PYTHONPATH=<workdir>:<workdir>/../taniteval`** — **BOTH ROOTS** (changed 2026-08-17, E5).
`<workdir>` alone and the trainer dies with `ModuleNotFound: tanitad`; without the **sibling**
`taniteval` root, `--dump-seam-plan`'s `import taniteval.seam_dump` fails, the trainer's broad
`except` prints *"training continues"*, and **X2_seam banks nothing, silently, at every save
boundary — first attempt ~1.8 h in**. The trainer now REFUSES that at startup instead (exit 2). ⛔ **`OMP_NUM_THREADS=6`** — torch spawns ~113 threads per process; 7 concurrent arms sat at
...
mkdir -p /root/experiments/v6F-SW-30k && cd /root/TanitAD/stack && PYTHONPATH=/root/TanitAD/stack:/root/TanitAD/taniteval OMP_NUM_THREADS=6 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True setsid nohup python3 -u scripts/train_v6_staged.py --stage S-W --out /root/experiments/v6F-SW-30k --steps 30000 --batch 8 --lr 0.0001 --n-candidates 8 --a-max 4.0 --adapter-hidden 512 --d-goal-embed 128 --d-str 512 --d-tac 768 --dt 0.1 --ema-decay 0.996 --enc-depth 12 --enc-dim 768 --enc-grad-checkpoint auto --enc-heads 12 --f-blocks 6 --f-hidden-str 1024 --f-hidden-tac 1024 --frame-h 256 --frame-w 640 --horizons 1 2 4 --in-channels 9 --kappa-max 0.2 --n-registers 4 --param-budget 350000000 --patch 16 --plan-steps 60 --pred-depth 12 --pred-dim 1024 --pred-heads 16 --pred-modern --readout-dim 128 --readout-grid 4 --sigreg-free-dims 0 --sigreg-slices 512 --uplink stopgrad --vit5-encoder --window 6 --v2-cache /root/data/physicalai-train-e438721ae894-w120-256x640cyl --v2-val-cache /root/data/physicalai-val-0c5f7dac3b11-w120-256x640cyl --v2-lru 6 --save-every 250 --frame-hfov 120 --projection cylindrical --require-parity > /root/experiments/v6F-SW-30k/train.out 2>&1 < /dev/null &

# ---- S-T · S-T · 10000 steps · 3.15 d at 27.18 s/step ----
# ⭐ THE NEXT LAUNCH. --selector none / --w-select 0: SEL-1 is REFUSED (§2.6).
# --tac-goal-cond builds the F-1 g_str->P_T port (zero-init; S-T introduces it).
# --dump-seam-plan banks the 60-step plan for X2_seam at every save boundary (ZERO extra GPU).
mkdir -p /root/experiments/v6F-ST-10k/seam && cd /root/TanitAD/stack && PYTHONPATH=/root/TanitAD/stack:/root/TanitAD/taniteval OMP_NUM_THREADS=6 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True setsid nohup python3 -u scripts/train_v6_staged.py --stage S-T --out /root/experiments/v6F-ST-10k --steps 10000 --batch 8 --lr 0.0001 --n-candidates 8 --a-max 4.0 --adapter-hidden 512 --d-goal-embed 128 --d-str 512 --d-tac 768 --dt 0.1 --ema-decay 0.996 --enc-depth 12 --enc-dim 768 --enc-grad-checkpoint auto --enc-heads 12 --f-blocks 6 --f-hidden-str 1024 --f-hidden-tac 1024 --frame-h 256 --frame-w 640 --horizons 1 2 4 --in-channels 9 --kappa-max 0.2 --n-registers 4 --param-budget 350000000 --patch 16 --plan-steps 60 --pred-depth 12 --pred-dim 1024 --pred-heads 16 --pred-modern --readout-dim 128 --readout-grid 4 --sigreg-free-dims 0 --sigreg-slices 512 --uplink stopgrad --vit5-encoder --window 6 --init-from /root/experiments/v6F-SW-30k/ckpt.pt --prev-gate /root/experiments/v6F-SW-30k/stage_gate.json --max-horizon 60 --tac-goal-cond --plan-wta-eps 0.05 --w-t1 1.0 --v2-cache /root/data/physicalai-train-e438721ae894-w120-256x640cyl --v2-val-cache /root/data/physicalai-val-0c5f7dac3b11-w120-256x640cyl --v2-lru 6 --save-every 250 --frame-hfov 120 --projection cylindrical --require-parity --dump-seam-plan /root/experiments/v6F-ST-10k/seam > /root/experiments/v6F-ST-10k/train.out 2>&1 < /dev/null &

# ---- S-S · S-S · 8000 steps · 2.52 d at 27.18 s/step ----
# ⛔ Its gate REQUIRES sel_gap_revalidated + TACTICAL_revalidated — see §2.7.
mkdir -p /root/experiments/v6F-SS-8k/seam && cd /root/TanitAD/stack && PYTHONPATH=/root/TanitAD/stack:/root/TanitAD/taniteval OMP_NUM_THREADS=6 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True setsid nohup python3 -u scripts/train_v6_staged.py --stage S-S --out /root/experiments/v6F-SS-8k --steps 8000 --batch 8 --lr 0.0001 --n-candidates 8 --a-max 4.0 --adapter-hidden 512 --d-goal-embed 128 --d-str 512 --d-tac 768 --dt 0.1 --ema-decay 0.996 --enc-depth 12 --enc-dim 768 --enc-grad-checkpoint auto --enc-heads 12 --f-blocks 6 --f-hidden-str 1024 --f-hidden-tac 1024 --frame-h 256 --frame-w 640 --horizons 1 2 4 --in-channels 9 --kappa-max 0.2 --n-registers 4 --param-budget 350000000 --patch 16 --plan-steps 60 --pred-depth 12 --pred-dim 1024 --pred-heads 16 --pred-modern --readout-dim 128 --readout-grid 4 --sigreg-free-dims 0 --sigreg-slices 512 --uplink stopgrad --vit5-encoder --window 6 --init-from /root/experiments/v6F-ST-10k/ckpt.pt --prev-gate /root/experiments/v6F-ST-10k/stage_gate.json --tac-goal-cond --w-s1 1.0 --v2-cache /root/data/physicalai-train-e438721ae894-w120-256x640cyl --v2-val-cache /root/data/physicalai-val-0c5f7dac3b11-w120-256x640cyl --v2-lru 6 --save-every 250 --frame-hfov 120 --projection cylindrical --require-parity --dump-seam-plan /root/experiments/v6F-SS-8k/seam > /root/experiments/v6F-SS-8k/train.out 2>&1 < /dev/null &

# ---- S-J · S-J · 3000 steps · 0.94 d at 27.18 s/step ----
# OPTIONAL. Run only if S-T/S-S plateau.
mkdir -p /root/experiments/v6F-SJ-3k/seam && cd /root/TanitAD/stack && PYTHONPATH=/root/TanitAD/stack:/root/TanitAD/taniteval OMP_NUM_THREADS=6 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True setsid nohup python3 -u scripts/train_v6_staged.py --stage S-J --out /root/experiments/v6F-SJ-3k --steps 3000 --batch 8 --lr 3e-05 --n-candidates 8 --a-max 4.0 --adapter-hidden 512 --d-goal-embed 128 --d-str 512 --d-tac 768 --dt 0.1 --ema-decay 0.996 --enc-depth 12 --enc-dim 768 --enc-grad-checkpoint auto --enc-heads 12 --f-blocks 6 --f-hidden-str 1024 --f-hidden-tac 1024 --frame-h 256 --frame-w 640 --horizons 1 2 4 --in-channels 9 --kappa-max 0.2 --n-registers 4 --param-budget 350000000 --patch 16 --plan-steps 60 --pred-depth 12 --pred-dim 1024 --pred-heads 16 --pred-modern --readout-dim 128 --readout-grid 4 --sigreg-free-dims 0 --sigreg-slices 512 --uplink stopgrad --vit5-encoder --window 6 --init-from /root/experiments/v6F-SS-8k/ckpt.pt --prev-gate /root/experiments/v6F-SS-8k/stage_gate.json --max-horizon 60 --tac-goal-cond --w-t1 1.0 --w-s1 1.0 --plan-wta-eps 0.05 --v2-cache /root/data/physicalai-train-e438721ae894-w120-256x640cyl --v2-val-cache /root/data/physicalai-val-0c5f7dac3b11-w120-256x640cyl --v2-lru 6 --save-every 250 --frame-hfov 120 --projection cylindrical --require-parity --dump-seam-plan /root/experiments/v6F-SJ-3k/seam > /root/experiments/v6F-SJ-3k/train.out 2>&1 < /dev/null &
```
...
| ⛔ **`--v2-subframe 176x624` REMOVED** (2026-08-17) | **In `train_v6_staged` the sub-frame moves the DATA and NOT the MODEL.** `resolve_v2_frames`'s docstring says the frame is applied to `cfg` "so the ENCODER is sized for what it will be fed" — true for `train_flagship_v4`, whose `cfg` **is** the model config, and FALSE here: this trainer passes a **flagship-v4 EVAL config**, while `build_stack_from_args` sized the encoder from `--frame-h/--frame-w` 120 lines earlier. ⚠️ **The dangerous part is the ORDER:** `--init-from` SUCCEEDS, `assert_v2_geometry_matches` PASSES, the corpus mounts — and the run dies at the FIRST FORWARD with `encoder input is (176, 624) but the config declares (256, 640)`. The live S-W run carries `v2_subframe: null`. | MEASURED 2026-08-17 on the built production encoder; `train_v6_staged.subframe_desync` now refuses it at STARTUP, args-only, in milliseconds |
| ⭐ **`--dump-seam-plan <out>/seam` on S-T/S-S/S-J** (added 2026-08-17) | The **only** thing that feeds `X2_seam`, and **zero extra GPU** — `L["out"]["plan"]` is already computed for the step's loss, so this copies it to CPU at the checkpoint boundary. F-16's probe had produced **zero real-arm numbers three times running** because nothing turned it on. ⚠️ **Deliberately NOT on S-W:** its emission head is at zero-init, every control is exactly (0, 0), and `seam_dump_from_plan` correctly refuses the plan as DEGENERATE. | `…/2026-08-17-st-launch-fixes/ST_LAUNCH_FIXES.md` §5 |
| ⚠️ **`--save-every 250` now explicit** | The trainer's own default is **1000**. The live S-W run uses 250, and the **seam dump fires at exactly this boundary**, so the default would quarter the number of banked plans without anyone choosing that. | `v6_chain.ChainConfig.save_every` |
| **`--tac-goal-cond` on every stage after S-W** (added 2026-08-16) | ⛔ **F-1, the diagram-conformance audit's top finding:** the binding diagram, `HIERARCHY_VOCABULARY` §5 and `V6Stack`'s own docstring all spec **`P_T(z_tac, a_tac \| g_str)`**, and the code never built the port — an S-T launched without it would never train the strategic→tactical **dynamics** downlink in its own stage (the `intent_proj` defect one level up). The flag builds `cond_tac_dyn`, a **zero-init** `Linear(d_goal_embed → 2·d_goal_embed)` added to the action-pair conditioning (+33,024 params, the intent-port idiom one level down); `STAGE_MAY_INTRODUCE["S-T"]` admits its fresh keys over the S-W checkpoint, so the introduction is loss-continuous. **S-S/S-J carry the flag as GEOMETRY** (like `--selector`): drop it above S-T and `--init-from` dies on unexpected `cond_tac_dyn.*` keys — `assert_geometry_carry` refuses that from a JSON read first. ⛔ Refused in S-W (would break the live run's strict resume). | `DIAGRAM_CONFORMANCE.md` F-1; `…/2026-08-16-gstr-port/GSTR_PORT.md`; `tests/test_v6_gstr_port.py` |
