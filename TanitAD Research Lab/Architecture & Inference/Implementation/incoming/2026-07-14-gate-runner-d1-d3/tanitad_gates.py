...
# --------------------------------------------------------------------------- #
# Thresholds (Phase 0 Plan §4). Kept as named constants, not magic numbers.   #
# --------------------------------------------------------------------------- #
D1_ADE_MAX = {"bev": 0.5, "camera": 1.0}   # metres, ADE@1s
D2_DIR_ACC_MIN = 0.7                        # calibrated direction accuracy
D2_IMAG_REL_MAX = 0.8                       # imag-rel (stricter than the generic I4<1)
D3_RATIO_MAX = 1.5                          # imagined-ADE@2s / oracle-ADE@2s
I1_FLOOR = 0.9                              # oracle-decode R^2 sanity floor
I2_TOL = 1e-4
I4_FLOOR = 1.0                              # persistence bar (D-004 I4)
...
def run_d2(z_prev: Tensor, z_true: Tensor, z_imag: Tensor, disp_true_xy: Tensor,
           ...
           disp_persist_xy: Tensor | None = None, alpha: float = 1e-3,
           val_frac: float = 0.2, seed: int = 0,
           extra_metrics: Mapping[str, Callable] | None = None) -> GateReport:
    """D2: does the imagined (action-conditioned) latent decode to the right
    direction well enough to *rank* candidate maneuvers?

    ``z_imag`` [N, S] imagined next-state, ``z_true`` [N, S] real next-state,
    ``z_prev`` [N, S] current state, ``disp_true_xy`` [N, 2] true ego displacement.
    probe_imag is calibrated on imagined latents (A3). Persistence baseline via
    ``disp_persist_xy`` (else chance = 0.5).
    """
    tr, va = split_by_episode(episode_ids, val_frac, seed)
    ...
    # baselines (I4 persistence is instrumental; direction persistence is a reference)
    ...
    rows = [_i1_row(fit_r2), _i2_row(i2), _i3_row(episode_ids, tr, va),
            _i4_row(z_imag, z_true, z_prev, ceil=D2_IMAG_REL_MAX)]
    admissible, blockers = _admissible(rows)
    passed = admissible and dir_acc > D2_DIR_ACC_MIN

    metrics = {"direction_acc": dir_acc, "mean_cos": float(cos.mean()),
               "baseline_persistence_dir_acc": base_dir_acc,
               "beats_persistence": bool(dir_acc > base_dir_acc)}
    if extra_metrics:
        metrics.update({k: fn(pred, truth) for k, fn in extra_metrics.items()})

    verdict = (f"BLOCKED: instruments failed {blockers}" if not admissible else
               f"{'PASS' if passed else 'FAIL'}: dir-acc={dir_acc:.3f} vs >{D2_DIR_ACC_MIN} "
               f"(persistence {base_dir_acc:.3f}). imag-rel bar {D2_IMAG_REL_MAX}.")
    return GateReport("D2", "imagination usable for selection", admissible, passed,
                      rows, metrics,
                      {"dir_acc_min": D2_DIR_ACC_MIN, "imag_rel_max": D2_IMAG_REL_MAX},
                      {}, blockers, verdict)
