...
class SitHead(nn.Module):
    def __init__(self, in_dim, d=128, n_out=len(SITS), dropout=0.2):
        super().__init__()
...
def run_fold(X, S, tr_idx, te_idx, cfg, epochs, device, seed=0, bank=None, batch=1024):
    ...
    pw = torch.full((len(SITS),), float(cfg["pw"]), device=device)
    ...
    out = np.empty((epochs, len(te_idx), len(SITS)), np.float32)
...
def ridge_fit_predict(Xtr, ytr, vtr, Xte, lam):
    ...
    Returns [n_te, len(SITS)] scores. Standardisation uses TRAIN rows only."""
    ...
    out = torch.zeros(len(Xte), len(SITS), dtype=torch.float64, device=Xte.device)
    assert out.shape[1] == ytr.shape[1] == vtr.shape[1] == len(SITS), "WIDTH MISMATCH: a hardcoded situation count survived. Five spellings were fixed one at a time; derive every width from len(SITS)."
...
def main():
    ...
    for arm in RIDGE_ARMS:
        ...
        best = None
        for lam in LAMBDAS:
            oof = np.full((len(tr_idx), len(SITS)), np.nan, np.float32)
            for f in range(a.folds):
                m = fold_of == f
