#!/usr/bin/env bash
# ⛔ DEVIATION D-3, DECLARED: this roll EXCEEDS the pre-registered budget.
# §14's drop order cut it at 2.85 h and §16.3 recorded H-DDV2RL-2 UNEVALUABLE.
# Then the l1-rl-s0 T1 read came back SEPARABLY WORSE THAN THE COLD START ON ALL TEN
# METRICS across all four families -- ade_m 0.2994 -> 0.5502 -- i.e. ~3x the harm the
# RELEASE form showed (+0.083 m), on the PRIMARY tier. The drop order was written
# without knowing the surviving seed would look like that.
# ⇒ shipping a package whose primary SAFETY endpoint is unevaluable, while its one
# measured seed shows 3x the known harm, is a worse outcome than a 25% overrun on an
# idle local GPU. ⛔ The CRITERION is untouched -- §13.3 still needs both seeds and
# still cannot be 'passed'; only the resource bound is exceeded, and it was MY bound,
# not a PI constraint. Declared here and in §16.4, never quietly.
