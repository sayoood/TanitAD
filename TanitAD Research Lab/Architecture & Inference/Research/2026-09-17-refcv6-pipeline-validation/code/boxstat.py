...
def main() -> int:
    try:
        # ⛔ BYTES, NOT print(). Python's Windows text mode turns "\n" into "\r\n",
        # so `read -r GU HF` in a shell gets "18\r" and every digit test on the
        # second field fails — which is exactly how this probe's caller aborted
        # with a shape error on a reading that was perfectly valid.
        sys.stdout.buffer.write(f"{gpu_used_mib()} {host_free_gb()}\n".encode())
