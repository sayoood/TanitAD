...
- [2026-09-05] [PUBLISHED-CODE `hustvl/DiffusionDriveV2@1cd12a1` (read in full, sha256 per file banked) + PUBLISHED-PRIMARY lib `2512.07745`] ⭐⭐ **THE RELEASED DiffusionDriveV2 CODE SETTLES EVERY FACT THE PAPER LEFT OUT — AND DEPARTS FROM THE PAPER IN NINE PLACES THAT DECIDE WHAT PORTS.** Settled (the 2026-08-29 RL-library research recorded these as `MEASURED-ABSENCE` when the code was "will be available"): the reward IS NAVSIM's PDM score from `PDMSimulator`/`PDMScorer` (`_model_rl.py:741-753`, `_pairwise_scores:75-117`: `NC×DAC×(5·EP+5·TTC+2·C)/12`, progress re-normalised against the GT's own progress); the `collision` predicate of Eq. 10 is `no_collision != 1 OR drivable_area != 1` (`:900-902`); G = 4 (`_rl_config.py:37`); RL rollout T = 10 DDIM steps `[18,16,…,0]` from noise at t = 8 (`:801-819`); **trunk + agent/BEV heads frozen AND in eval mode, only `_trajectory_head` trains** (`_rl_agent.py:61-67`); top-k = 32; margin m = 0.05 with rank weight 2 (`_model_sel.py:904,1021`). Departures: (1) the RL and selector decoders are **ONE cascade layer** (`:733`, `_sel.py:844`) vs v1's two, loaded `strict=False` so v1's second layer is dropped — "matching the decoder size of DiffusionDrive" (§5.2) is not what ran; (2) the paper's "η = 1 DDPM" exploration is **only** a 2-scalar (longitudinal, lateral) multiplicative scale — `std_dev_t_add = 0.0` (`:640,:666`); (3) `ESTIMATED` from the diffusers schedule, σ_t(η=1) ≤ 0.012 at every rollout step, so **both floors bind everywhere**: exploration std is a constant 4 % of the coordinate and the likelihood std a constant 0.1 normalised = 5 m (x) / 2 m (y) — the schedule is irrelevant; (4) the likelihood is an isotropic Gaussian summed over 16 coordinates on samples drawn from a 2-scalar multiplicative law — **not the sampler's density**; with `exp(logp − logp.detach())` the ratio ≡ 1: plain REINFORCE, one update per rollout, **no clip, no KL, no reference policy** (`:1096`); (5) positive advantage ONLY for samples whose PDM score ≥ the human trajectory's own (`mask_positive`, `:892-893`) — absent from Eq. 10; (6) the IL term is L1 of **all 80 chains** to the GT at every step and layer, λ = 0.1 or **1.0 when no sample beat the human** (`:1104-1118`) — a global pull, not v1's nearest-anchor L1; (7) v1's confidence head is computed, untrained and unread — selection is the stage-II scorer over **800 candidates** (10 groups × 20 anchors × (1 + 3 multiplicative augmentations U(0.1, 0.3))) → coarse (1 layer, d 512, 16 heads, self-attention among candidates) → top-32 → fine (3 layers), each with five PDM-sub-score heads whose sigmoid outputs are recombined by the PDMS formula (`_sel.py:1461`); (8) `norm_odo` is `x/50, y/20` vs v1's `2(x+1.2)/56.9−1` — the cold start sees t = 8 start noise of 1.58 / 0.63 m instead of the 0.90 / 0.73 m it was trained with; (9) the official-eval return is commented out (`_sel.py:1491`) and `run_pdm_score_fast.py` scores the picks against the metric cache — `UNVERIFIED` which path produced Table 1.
  impact: **DDv2's RL stage is, in the released code, a policy over TWO SCALARS per anchor (along-track scale, lateral scale) — it never uses the diffusion chain's randomness, so a DD-faithful sampler (E-DDA-3) is NOT the prerequisite for testing its lever on REF-C; E-DDA-3 is demoted to the prerequisite for the paper's per-step chain objective and for diversity claims. Two code-only ingredients are missing from `stack/tanitad/rl/` — the ≥ GT bar and the 2-scalar noise mode — and both are one-line adds** — `Architecture & Inference/Research/2026-09-05-diffusiondrive-v2-analysis/RESULT.md` §1.2–1.3; register `D-DDV2-CODE-1..5`

- [2026-09-05] [PUBLISHED-PRIMARY lib `2512.07745` Tab. 1/2/3/4–6/8/9, p. 7–8, 12–13] **THE V2 NUMBERS, WITH THE CAVEAT THAT TRAVELS WITH EACH**: NAVSIM v1 navtest PDMS **91.2** (NC 98.3 · DAC 97.9 · TTC 94.8 · Comf 99.9 · **EP 87.5**) vs DiffusionDrive 88.1 — **+3.1, of which EP +5.3 and DAC +1.7; NC/TTC/Comf move ≤ 0.1**; beats DriveSuprim 89.9, WoTE/DIVER 88.3 at ResNet-34 and Hydra-MDP/GoalFlow 90.3 on the 4.4×-larger V2-99. NAVSIM v2 **navtest** EPDMS **85.5** (single-stage extended metric — NOT navhard two-stage; the Ego-Status-MLP 64.0 in that table is the navtest figure) with the field's largest margin on **EC 91.0 vs 77.0**. Raw fan before any selector (Tab. 3, 20 trajectories): diversity **42.3 → 30.3 (−28 %)**, PDMS@1 93.5 → 94.9, **@10 75.3 → 84.4** — the floor moved 6.5× more than the top. The authors' own control (Tab. 9): DD + V2's selector **89.1** (NC 98.2 → 97.2, TTC 94.7 → 92.2, EP 82.2 → 86.8 — progress bought with collisions) → V2 91.2 ⇒ **selector +1.0, RL ≈ +2.1**. Ablation deltas sit on a reduced-epoch 90.1 base and are never additive with 91.2: multiplicative-vs-additive noise +0.4, intra-anchor grouping +0.9, inter-anchor truncation +0.6, coarse-to-fine +0.2, rank loss +0.2. **Never ablated: the reward, the ≥ GT mask, the all-modes IL pull, the one-layer decoder, G, the 800-candidate fan. Every number is single-run with no interval.** ⚠️ The paper's "closed-loop evaluation" is NAVSIM's 4-s LQR rollout against **non-reactive replayed agents** (NAVSIM's own title, V2 ref. [10]) — under the PI's 2026-09-02 ruling that is **self-action open loop, our T1 class**, not closed loop.
  impact: **quote 91.2 (P1) and 85.5 (P4 navtest) with their protocols and never merged; size an RL post-train on ≈ +2.1 and a selector on +1.0, not on +3.1; expect any fan-floor reward to narrow the fan; and treat V2's tier as comparable to our T1, which repairs the "their closed-loop vs our open-loop" framing** — `…/2026-09-05-diffusiondrive-v2-analysis/RESULT.md` §2; register `D-DDV2-RES-1`

- [2026-09-05] [DERIVATION over PUBLISHED-CODE `1cd12a1` + MEASURED `D-REFCV3-AXIS1` / `D-REFCV4-GATE1` + INHERITED `H-RL-REACH-1`] ⭐ **V2's RL LEVER AND OUR FAN'S DEFICIT ARE THE SAME DIMENSION — AND THE ADMISSIBLE REWARD ON PhysicalAI IS NAVSIM's STRUCTURE WITH ITS MAP HALF REMOVED.** Because the additive branch is zeroed and both σ floors bind, V2's RL explores only (longitudinal scale, lateral scale) per anchor, and its published gain is progress (+5.3 EP) and drivable-area (+1.7 DAC) with no movement in collision/TTC; `D-REFCV3-AXIS1` puts 92.2 % of our `os − ha` gap along-track (`corr(Δv)` 0.555 vs hold-action 0.851). ⇒ a per-anchor (a_long, a_lat) scale policy over refcv4's DETERMINISTIC fan is the code-faithful port and needs no diffusion chain. Reward taxonomy by what a term READS: ego GT future (ADE/FDE, speed-profile match, "stay near the recorded path", route agreement) = **ECHO — the IL loss in a reward costume**; a label derived from the ego future (v7.2 `a_tac` class agreement) = partial echo, train-side only, never grouping-key AND reward at once; other agents' futures (collision/TTC/headway vs replayed `obstacle.offline`), candidate geometry (comfort, feasibility, progress) and the GT's SCORE (V2's ≥ GT bar) = admissible. V2 itself brushes the trap: EP is normalised by the human's progress and the GT enters both the reward and the IL term — the pattern the 2026-08-29 research flagged in OUR library (E.2) is in the published code too. Admissible composition: `NC(replay) × (5·TTC/headway + 2·comfort + 5·progress)/12`, ≥ GT bar, collision → −1, T0 training-side; the capability claim is T1 with the four families. NAVSIM's five sub-scores map onto only TWO of our families (NC/TTC → LONGITUDINAL distance-keeping; C → comfort/LATERAL); DAC/LK need a map we lack; **TACTICAL and STRATEGIC have no NAVSIM analogue — v7.2 labels supply what NAVSIM cannot.**
  impact: **H-DDA-3 is demoted from "the GRPO prerequisite"; the decision ladder is E-DDA-3b (scale policy + ≥ GT bar on the P-RC21 rig, pre-registered per-axis) → E-DDA-2b (V2-style sub-metric selector on the emitted fan, four-family heads, deliberate-regression = progress-only) → E-DDA-6 (grouping by v7.2 tactical class vs by anchor vs pooled) → only then E-DDA-3; P-RC21's "the reward pays for drift" ran without the two ingredients that keep V2 on the road (the ≥ GT bar that sets λ_IL = 1.0 on most rows, and scale-only exploration that cannot leave the road by construction)** — `…/2026-09-05-diffusiondrive-v2-analysis/RESULT.md` §3–4; register `H-DDA-5..7`, `D-DDV2-ORDER-1`
...
- [2026-09-03] [MEASURED ours `Architecture & Inference/Research/2026-09-03-ego-zero-collision/raw/`] **THE ZERO-COLLISION DEFECT HAS A SECOND INSTANCE, ON THE EGO-SPEED CHANNEL, AND THE COMPANION BIT THAT FIXES IT IS ALREADY COMPUTED AND ALREADY USED ELSEWHERE IN THE SAME FORWARD**: refcv3 trains with `ego_dropout = 0.5` (`refc.py:2032`) and `ego_valid_channel = False` (`refc.py:585`, gate at `:2060`), so a WITHHELD speed and a GENUINE 0.0 m/s standstill are BYTE-IDENTICAL at the measurement encoder - confirmed live by three probes of different path binding (the run’s `config.json` stamps neither knob, the trainer has no CLI flag for either, and the pod’s `refc.py` is md5-identical to the repo). MEASURED consumer-exact over the trainer’s own enumeration (781,635 windows / 4,572 clips, from the `_v2manifest.pt` the provider hands the dataset): **v0 is exactly 0.0 on 4.4531 %** (34,807; 6.548 % at ≤ 0.5 m/s), a hard ATOM 13× the next histogram bin, corroborated at 3.282 % over 14,094,549 raw egomotion rows. ⇒ **52.227 % of training samples present a zero speed and only 4.263 % of those are a real standstill (22.5 : 1)**, while at eval - dropout being training-only - the same token is 100 % genuine: a **23.5× shift in meaning**. The consequence is STRUCTURAL, not correlational: `refc_tactical.py:202-242` makes `lat = f(dyaw)` alone but `lon = f(dv, v0, v1)`, and its stop-branch needs `v0 ≥ 1.0` ⇒ **`brake_stop` is unreachable at v0 = 0, MEASURED 0 of 34,807**, so the token carries 16.0 % brake mass at train and 0.0 % at eval (TV 0.2506 longitudinal vs 0.1542 lateral). ⛔ **The live run CANNOT test this**: `tactical_speed_input = False` (`refc_v3.py:213`) means the heads carrying the 5.3× lat/lon skill gap (era C: 0.7627 vs 0.1437 as `1 - CE/H(marginal)`, lat better on 38/38 rows) **never see the speed at all**, and the columns that DO consume it (`eval_traj`, `goal2s_err_m`) have no longitudinal/lateral decomposition. refav1 avoids the defect entirely - no dropout, no zero-fill, and `v0=None` RAISES (`refa_v1.py:1343`).
  impact: **a one-flag fix on a tensor the model already computes and already passes to the decoder as `ego_keep` (honoured at `refc.py:1354`/`:1494`) - pre-registered as `H-ARCH-EGOZERO-1` with a 5-arm panel whose deliberate-regression arm feeds a PERMUTED bit so a win cannot be width; and two work items that stand regardless of the outcome: refcv3’s `config.json` stamps neither ego knob (refav1’s stamps `speed_channel`), and the in-training eval has no longitudinal family and no per-window dump, so no CI is computable from it by any estimator** - `Architecture & Inference/Research/2026-09-03-ego-zero-collision/RESULT.md`

- [2026-09-03] [PUBLISHED-PRIMARY lib `2206.08129` `2411.15139`] **"THE ORIGINAL REF-C PAPER" IS TWO PAPERS, AND ONLY THE TCP HALF CARRIES THE EGO — WHICH SETTLES WHERE OUR RISK IS**: `refc.py:1-13` names both ancestors, and TCP (`2206.08129` §3.2 + Tab. 5) feeds the ego speed into the PLANNER path unguarded — `m = [g, v]` -> an MLP measurement encoder of **FC-128 x2 (bit-for-bit our `MeasurementConfig(hidden=128, d_out=128)`)** -> `j_m`, concatenated with the pooled image map into `j_traj` for the trajectory GRU **and** the control branch; no head is denied it, there is no presence bit, and **TCP has NO dropout of any kind (two probes: full-text grep for `dropout|drop out|mask| drop ` = 0 hits; its own layer table lists the measurement encoder with no regulariser)**. DiffusionDrive (`2411.15139` §3.4, §4.2, App. A) has **no ego port in the decoder at all** — its condition is scene context, and ego status, where it exists, comes from the host backbone (Transfuser/SparseDrive), **never documented** (its Tab. 3 "Ego Query" column is ✓ in all 6 rows, i.e. never ablated). ⛔ **Consequence: `refc.py:10-11`'s "the measurement encoder with per-sample ego-dropout [kept verbatim]" LAUNDERS AN UNSWEPT HYPER-PARAMETER AS INHERITED PRACTICE** — `ego_dropout = 0.5` is a TanitAD invention nobody upstream ever validated. And **our edge E11 (no ego into `{z_tac, g_str, ĝ_tac}`) has NO ancestor precedent**: of the 8 planners whose primaries we now hold, 7 feed ego status and **none** withholds it from a planner or goal head by design.
  impact: **the docstring correction is a work item in the un-refined-anchor class (a true statement phrased so a reader infers a false provenance), and E11 must be RE-SCOPED from an absolute prohibition to a stated boundary or the PI's 2026-09-03 instruction cannot be implemented** — `Architecture & Inference/Research/2026-09-03-refc-ego-inputs-and-anti-echo/RESULT.md` §1-§3, `COMMS.md` C-1/C-3

- [2026-09-03] [PUBLISHED-PRIMARY lib `2312.03031` `2305.10430` `paradrive-cvpr2024` `2406.15349` `2506.04218`] **A BLIND EGO-ONLY AGENT TIES THE BEST CAMERA PLANNERS ON nuScenes AND IS 4x BEHIND ON A FILTERED SPLIT — THE BENCHMARK, NOT THE MODEL, DECIDES WHETHER THE ECHO IS VISIBLE**: Ego-MLP (velocity, acceleration, yaw angle, command; **no perception**) scores nuScenes **L2 avg 0.35 m** = BEV-Planner++ 0.35, beating VAD-with-ego 0.37 and UniAD-with-ego 0.46 (`2312.03031` Tab. 1); PARA-Drive reproduces it independently to four decimals on its own standardised protocol — **PARA-Drive without ego 0.5574 vs AD-MLP blind 0.5568** — while the SAME pair separates **10.1x on off-road (0.12 vs 1.21)**, **3.0x off-lane (0.83 vs 2.45)** and **6.7x on collisions in the non-straight subset (0.14 vs 0.94)**. On NAVSIM the ego-only MLP sits **18.4 PDMS behind** the sensor agents (65.6 vs 84.0) and on NAVSIM v2/navhard **4.0x behind** (**EPDMS 14.1 vs 56.6**) — *even though v2 gives the blind agent MORE ego (velocity AND motion history)*. ⭐ **The cause is corpus curation, not architecture**: NAVSIM removed scenes a constant-velocity agent already solves (PDMS > 0.8) and **the CV agent fell from 79 to 22 PDMS** — a larger effect than any architectural fix in the literature; nuScenes is **73.9 % straight**, 13 % turning, 87 % of eval samples straight. ⚠️ **AD-MLP's own published row is leak-contaminated** (past-ego-trajectory GT; re-implemented for that reason by BOTH BEV-Planner and PARA-Drive) and must never be quoted bare. ⚠️ **A circulating "Ego Status EPDMS 64.0 on navhard" is REFUTED — the primary says 14.1.**
  impact: **E-ECHO-2 (0 GPU, CPU-only) is now the gating experiment: score all 881 canonical val windows with a constant-velocity + constant-yaw-rate agent and build BOTH published hard splits (NAVSIM's CV-PDMS filter and PARA-Drive's label-only "drop keep-forward" -> 686 keyframes). If >=80 % of our windows are CV-trivial, EVERY number on this split is dominated by the trivial subset and the first deliverable is a split, not a loss** — `…/2026-09-03-refc-ego-inputs-and-anti-echo/RESULT.md` §4, `COMMS.md` H-2

- [2026-09-03] [PUBLISHED-PRIMARY lib `2312.03031` `2309.10443` `2010.14876` `2606.14438` `paradrive-cvpr2024`] **THE ECHO IS DETECTABLE ON A TRAINED CHECKPOINT FOR ZERO TRAINING COMPUTE, AND IT IS AN ASYMMETRY — NOT A SINGLE METRIC**: on VAD-Base **with** ego status, blanking every camera moves L2 **0.37 -> 0.46 (x1.24)** while detection NDS goes **45.5 -> 0.0** and map mAP **47.0 -> 0.0**, whereas scaling velocity x0.5 moves it **0.37 -> 3.19 (x8.6)** and v=0 gives **6.16 (x16.6)**; **the same architecture WITHOUT ego status** moves **1.25 -> 4.33 (x3.46)** under the identical blanking (`2312.03031` Tab. 2 + App. Tab. 3). Four instruments now have published reference values: **D1** blank-image ratio (echoing 1.24x / healthy 3.46x), **D2** ego-perturbation response, **D3** `‖∂(x_T,y_T)/∂s_0‖` — one backward pass, PlanTF Fig. 2(c)(d) — and **D4** the plan-predictability probe (a copycat is **~10x more self-predictable than the expert**; after the fix it moves back toward it, `2010.14876`). ⭐ **PARA-Drive Tab. 4 publishes our own required "constant-only control that must read the no-information value"**: a command-only planner reads Coll 5.88 / L2 4.66 against 0.13 / 0.53 with BEV. ⚠️ **AGGREGATION HIDES IT**: with cameras blanked and no ego, aggregate CCR moved **3.82 -> 3.81** (nothing) while **CCR-ST went 9.13 -> 17.6 and CCR-LR 3.05 -> 1.69** — a total behavioural collapse invisible to the mean. ⚠️ **And ADE structurally cannot do this job**: CADET measured that suppressing genuine spurious reliance *"moves the official open-loop metrics by under 0.1 m"*.
  impact: **the criteria registry has NO echo instrument at all; D1/D2/D3/D7 go in with published values to validate them against, and E-ECHO-1 (0 GPU-train, NOT on the training pod) reads refcv3's ratio today. ⛔ D6 (road-adherence / CCR / off-road) is UNAVAILABLE — PhysicalAI has no map, pinned by `test_physicalai_feature_readset.py` — which is exactly why the map-free ratio is the first instrument** — `…/2026-09-03-refc-ego-inputs-and-anti-echo/RESULT.md` §7, `COMMS.md` H-3/H-4

- [2026-09-03] [PUBLISHED-PRIMARY lib `2309.10443` `2408.03601` `2404.14327` `1606.01865`] **PER-SCALAR EGO DROPOUT + A PRESENCE BIT IS THE BEST-EVIDENCED FIX, ITS RATE IS PUBLISHED, AND IT COSTS OPEN-LOOP SCORE WHEN IT IS WORKING**: PlanTF's State Dropout Encoder gives each ego scalar its own embedding and drops **every token EXCEPT position and heading** — **OLS 88.55 -> 87.07 (−1.48) while NR-CLS 83.19 -> 86.48 (+3.29) and R-CLS 74.79 -> 80.59 (+5.80)**; rate sweep **0/0.25/0.50/0.75 -> NR-CLS 77.28/81.70/83.71/86.48**, deployed at **0.75**. Its no-kinematics arm (`state3`, pose only) has the BEST closed-loop score of the non-SDE models (85.99) and the WORST open-loop (81.13) — the inversion in one table. DRAMA reproduces it **on NAVSIM PDMS: 0.835 -> 0.848**, with a **differentiated policy — ego 0.5, perception features 0.1** — and PLUTO reuses the SDE unchanged for **+2.60 nuPlan score**. GRU-D is the primary for the presence bit (mask + time-since; **masking-only ablation 0.8367 vs GRU-mean 0.8192**) and its argument is the PI's verbatim: imputation *"cannot distinguish whether missing values are imputed or truly observed"*. ⚠️ **PlanTF exempts pose from dropout; ALL THREE of our planned inputs (`v`, `a_long`, `yaw_rate`) fall in the DROPPABLE class and none in the exempt one.** ⚠️ Counter-evidence held in the same hand: `1905.11979` measured that **plain dropout gives minimal improvement** on its confounded benchmarks — both primary, so the tension is a reason to run the arm, not to adjudicate in prose.
  impact: **`H-ARCH-EGOZERO-1` should SWEEP the rate rather than assume 0.5 (the two papers that swept it landed on 0.5 for PDMS and 0.75 for closed-loop), and ⛔ the refcv4 gate must NOT be scored on ADE — three independent primaries say a working guard lowers displacement error while raising closed-loop/tactical score** — `…/2026-09-03-refc-ego-inputs-and-anti-echo/RESULT.md` §6.1/§8, `COMMS.md` C-2/H-1

- [2026-09-03] [PUBLISHED-PRIMARY lib `2205.15997` `2511.13079` `paradrive-cvpr2024` `2411.15139`] **WHERE THE EGO ENTERS IS THE WHOLE ARGUMENT — AND refcv4's INTENDED PLACEMENT IS THE PROTECTED ONE**: the only CLOSED-LOOP measurement of harm is CARLA-TransFuser's Table 10 — **DS 56.68 -> 45.35 with creeping (−11.33), 46.35 -> 37.34 without (−9.01)**, *"a sharp drop in DS, which cannot be recovered"* — but its injection site is *"element-wise summation [with] the learnable positional embedding … fed into the transformer at **all 4 stages of the backbone**"*, i.e. **the SCENE ENCODER**, the exact site AdaptiveAD root-causes (*premature fusion in the upstream BEV encoder*) and the exact site PARA-Drive protects (ego into **the planner head only**, where the same information is *"marginal"* on val and useful on the hard split; NAVSIM's TransFuser gains only **1.5–2.6 PDMS** from it). ⇒ **quoting "velocity input hurts" without its attach point is the `df`/`step_s` scope error in a new costume.** ⛔ **AND ONE CANDIDATE IS REFUSED ON EVIDENCE**: predicting a residual over a constant-velocity extrapolation is NOT FOUND as a published driving method (2 phrasings), and the nearest measurement **in our own architecture class** contradicts it — DiffusionDrive App. Tab. 8: learned k-means anchors **88.1 PDMS**, trained-on-a-single-extrapolated-anchor **84.7**, extrapolated-prior-at-inference **81.3**, because the kinematic prior *"fails to cover the potential action space"*. The residual that DID work (`2207.09705`: **52.0 vs 41.3 absolute, 45.0 without stop-gradient**) sits on the auxiliary target of a memory branch, not on the plan's output parameterisation.
  impact: **keep refcv4's ego in the measurement encoder -> decoder condition and NEVER in the visual trunk, and say so in the design doc WITH the Tab. 10 citation — that placement is the difference between the +2 PDMS result and the −11 DS one; use constant-velocity as a REFERENCE (a CV-excess metric, or one anchor among 128) and never as the parameterisation** — `…/2026-09-03-refc-ego-inputs-and-anti-echo/RESULT.md` §5.1/§7.4, `COMMS.md` C-1
