"""MUTATION PROOF for `test_trunk_speed_levers.py`. Anchors are WHOLE LINES.

  L1  the autocast ignores the flag (bf16 stamped, fp32 run)
  L2  the NHWC conversion is removed
  L3  the backbone outputs are NOT returned to float32
  L4  the trainer does not pin --trunk-bf16 into the config
  L5  build_encoder does not pass the flag to the trunk
"""
...
TESTF = REPO / "stack" / "tests" / "test_trunk_speed_levers.py"
...
MUTATIONS = [
    ("L1_autocast_ignores_the_flag", TT,
     '                            enabled=bf):',
     '                            enabled=False):'),
    ("L2_nhwc_conversion_removed", TT,
     '            self.net.to(memory_format=torch.channels_last)',
     '            pass'),
    ("L3_outputs_not_returned_to_fp32", TT,
     '        return [o.float().contiguous() for o in out]',
     '        return list(out)'),
    ("L4_trainer_does_not_pin_the_flag", TRAIN,
     '    cfg.core.encoder.trunk_bf16 = bool(getattr(args, "trunk_bf16", False))',
     '    cfg.core.encoder.trunk_bf16 = False'),
    ("L5_build_encoder_drops_the_flag", RC,
...
def main() -> int:
    files = {TRAIN, TESTF, TT, RC}
    orig = {p: p.read_bytes() for p in files}
    md5 = {p: hashlib.md5(b).hexdigest() for p, b in orig.items()}
    res = {"_what": "mutation proof that test_trunk_speed_levers.py can FAIL",
    ...
    try:
        ...
        for name, target, old, new in MUTATIONS:
            ...
            if len(hits) != 1:
                ...
                pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_trunk_speed_levers.json"
    ...
    res["_VERDICT"] = (
        ...
        "a lever stamped but not run; all files restored "
    ...
    pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_trunk_speed_levers.json").write_text(
