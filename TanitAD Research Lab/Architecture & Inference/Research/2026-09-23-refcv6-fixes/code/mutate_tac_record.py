"""MUTATION PROOF for the tactical RUN-RECORD tests in `test_refcv6_bev_tactical_wiring.py` (section 7). Anchors are WHOLE LINES.

Arms -- each reintroduces the defect the section exists for, or a neighbour of it:
  T1  the helper writes the historical LITERAL live-sources list
  T2  bev counted live from the WIDTH alone (no branch attached)
  T3  bev forced live on every arm
  T4  train() stops calling the builder (the call-site regression)
  T5  the blocked list goes back to the literal ["bev"]
...
TESTF = REPO / "stack" / "tests" / "test_refcv6_bev_tactical_wiring.py"
TESTS = REPO / "stack" / "tests" / "test_refcv6_bev_tactical_wiring.py"
...
MUTATIONS = [
    ("T1_helper_writes_the_historical_literal", TRAIN,
     '        "scene_sources_live": [s for s in sources if s != "bev" or bev_live],',
     '        "scene_sources_live": ["agent"],'),
    ("T2_bev_live_from_width_alone_no_branch_check", TRAIN,
     '    bev_live = "bev" in sources and d_bev > 0 and branch is not None',
     '    bev_live = "bev" in sources and d_bev > 0'),
    ("T3_bev_forced_live_on_every_arm", TRAIN,
     '    bev_live = "bev" in sources and d_bev > 0 and branch is not None',
     '    bev_live = True'),
    ("T4_train_stops_calling_the_builder", TRAIN,
     '        "refcv6_tactical": _refcv6_tactical_block(args, model, tac_goal_stats),',
     '        "refcv6_tactical": None,'),
    ("T5_blocked_list_back_to_the_literal", TRAIN,
...
def main() -> int:
    ...
    res = {"_what": "mutation proof that the tactical run-record tests can FAIL",
    ...
    try:
        ...
        for name, target, old, new in MUTATIONS:
            ...
            if len(hits) != 1:
                ...
                pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_tac_record.json"
    ...
    res["_VERDICT"] = (
        ...
        "the historical literal and the call-site regression; both files restored "
    ...
    pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_tac_record.json").write_text(
