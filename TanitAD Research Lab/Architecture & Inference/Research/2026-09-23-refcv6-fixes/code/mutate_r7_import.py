"""MUTATION PROOF for `test_trainer_imports_without_refcv7.py`, run on the CLEAN TREE
(a git archive of the tip plus this commit's files) -- the only tree where the refcv7
modules are genuinely absent. Anchors are WHOLE LINES.

  I1  the tolerance removed: the plain module-scope import comes back (d014414's state)
  I3  the --refcv7 refusal removed while the modules are missing
"""
...
REPO = pathlib.Path("C:/Users/Admin/tc0923")
TRAIN = REPO / "stack" / "scripts" / "refc_v3_train.py"
TESTF = REPO / "stack" / "tests" / "test_trainer_imports_without_refcv7.py"
TESTS = REPO / "stack" / "tests" / "test_trainer_imports_without_refcv7.py"
...
MUTATIONS = [
    ("I1_tolerance_removed_plain_import_back", TRAIN,
     '    if "refcv7_" not in str(_e_r7):',
     '    if True:'),
    ("I3_refcv7_refusal_removed", TRAIN,
...
def run_tests():
    r = subprocess.run(
        ...
        env={**{k: v for k, v in os.environ.items()
             ...
             "PYTHONPATH": str(REPO / "stack") + ";" + str(REPO / "taniteval"), "PATH": os.environ["PATH"],
...
def main() -> int:
    ...
    res = {"_what": "mutation proof that test_trainer_imports_without_refcv7.py can FAIL",
    ...
    try:
        ...
        for name, target, old, new in MUTATIONS:
            ...
            if len(hits) != 1:
                ...
                pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_r7_import.json"
    ...
    res["_VERDICT"] = (
        ...
        "the d014414 import and the missing refusal; both files restored "
    ...
    pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_r7_import.json").write_text(
