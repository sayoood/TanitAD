"""MUTATION PROOF for `test_resume_continues_the_data_order.py`. Anchors are WHOLE LINES.

Arms -- D3 IS THE HISTORICAL DEFECT (shuffle=True, replayed on every relaunch):
  D1  the sampler ignores the resume offset
  D2  the permutation ignores the epoch (every epoch identical)
  D3  the loader goes back to shuffle=True
  D4  resume does not restore the position
  D5  the checkpoint stops saving the position
  D6  the changed-run validation is removed
  D7  the rollover stops advancing the epoch

Harness body is mutate_route_gate.py verbatim (utf-8 child decode, named-test RED only,
non-unique anchor ABORTS as INVALID, verdict read off the JSON artifact).
"""
...
TESTF = REPO / "stack" / "tests" / "test_resume_continues_the_data_order.py"
TESTS = REPO / "stack" / "tests" / "test_resume_continues_the_data_order.py"
...
MUTATIONS = [
    ("D1_sampler_ignores_the_resume_offset", TRAIN,
     '        p = self.permutation(self.epoch)[self.skip_batches * self.batch:]',
     '        p = self.permutation(self.epoch)'),
    ("D2_permutation_ignores_the_epoch", TRAIN,
     '        g = torch.Generator().manual_seed(self.seed * 1_000_003 + int(epoch))',
     '        g = torch.Generator().manual_seed(self.seed * 1_000_003)'),
    ("D3_HISTORICAL_loader_back_to_shuffle_True", TRAIN,
    ...
    ("D4_resume_does_not_restore_the_position", TRAIN,
     '    sampler.skip_batches = int(dp["batch"])',
     '    sampler.skip_batches = 0'),
    ("D5_checkpoint_stops_saving_the_position", TRAIN,
     '                                     "batch": int(_dpos["batch"]),',
     '                                     "batch": 0,'),
    ("D6_changed_run_validation_removed", TRAIN,
     '           if dp.get(k) != v]',
     '           if False]'),
    ("D7_rollover_stops_advancing_the_epoch", TRAIN,
     '        sampler.epoch += 1',
     '        sampler.epoch += 0'),
]
...
def main() -> int:
    ...
    res = {"_what": "mutation proof that test_resume_continues_the_data_order.py can FAIL",
    ...
    try:
        ...
        for name, target, old, new in MUTATIONS:
            ...
            if len(hits) != 1:
                ...
                pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_resume_order.json"
    ...
    res["_VERDICT"] = (
        ...
        "the historical shuffle=True replay; both files restored "
    ...
    pathlib.Path("C:/Users/Admin/qland/work/pbox/mutation_proof_resume_order.json").write_text(
