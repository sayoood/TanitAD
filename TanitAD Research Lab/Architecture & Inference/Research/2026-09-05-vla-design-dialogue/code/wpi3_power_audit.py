...
JOIN = sys.argv[2] if len(sys.argv) > 2 else O + "/pilot_val_agents_ext.jsonl"
OUT = sys.argv[3] if len(sys.argv) > 3 else O + "/wpi3_power_audit.json"
VRU = {"person", "rider", "stroller", "animal"}

raw = collections.defaultdict(dict)
TS = collections.defaultdict(dict)
for line in io.open(JOIN, encoding="utf-8"):
    d = json.loads(line)
    raw[d["clip_id"]][d["frame_idx"]] = d["agents"]
    TS[d["clip_id"]][d["frame_idx"]] = float(d["t_s"])
_meta = json.load(io.open(JOIN + ".meta.json", encoding="utf-8"))
...
for stem in src.stems:
    ...
    for t in idx:
        ...
        if sp is not None and ts is not None and not (sp[0] <= ts <= sp[1]):
            continue                     # NO_LABEL is never a negative
