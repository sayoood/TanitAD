#!/bin/sh
# RESUME of run_validation.sh after DEVIATION D-1 (the NORL check's rl_part criterion was
# mis-specified; see check_arm.py). RL-s0 and NORL-s0 are complete and are NOT re-run:
# NORL-s0 is re-checked under the amended criterion, then the chain continues from RL-s1.
set -u
...
stamp "re-check (amended I2) norl s0"
"$PY" "$PKG/code/check_arm.py" "$OUT/arm-norl-s0" --steps 600 --arm norl || { echo "ARTIFACT CHECK FAILED: norl s0 (amended)"; exit 4; }
stamp "re-check rl s0"
"$PY" "$PKG/code/check_arm.py" "$OUT/arm-rl-s0" --steps 600 --arm rl || { echo "ARTIFACT CHECK FAILED: rl s0"; exit 4; }

A=rl; S=1; D="$OUT/arm-$A-s$S"
gpu_free
stamp "train $A s$S"
