...
W = Path(r"C:\Users\Admin\run_refcv4v")
...
def build_model(arm_dir: Path):
    ...
    # ⛔ RESTORE THE ARM'S OWN REACH BAND. Without this every arm is scored at
    # the 2.5 DEFAULT, so the C/D/E arms -- which TRAINED at 2.0 -- would be read
    # through a band 1.25x wider than the one they learned under. The anchors
    # themselves ride in the state_dict (core.decoder.anchors is a buffer), so
    # they need no separate load; the band does not.
    _sel = cfg_j.get("selection") or {}
    if _sel.get("sel_accel_max") is not None:
        cfg.core.sel_accel_max = float(_sel["sel_accel_max"])
    if _sel.get("sel_reach_clamp") is not None:
        cfg.core.sel_reach_clamp = bool(_sel["sel_reach_clamp"])
