...
| | |
|---|---|
| **status** | ⛔⛔ **UNVERIFIED-CLAIM** — "solved" is true of one arm and false of every other arm ever measured |
...
| | |
|---|---|
| **status** | ⛔⛔ **UNVERIFIED-CLAIM** — the closure arm RAN and was READ, and then the **metric itself** was invalidated |
...
| | |
|---|---|
| **status** | ⚠️ **PARTIAL → and the decisive level (L3) is UNDECIDED, never passed or failed on an admissible read** |
...
| 2 | **non-collapse** | ⛔⛔ **UNVERIFIED-CLAIM** | v7-tiny, **one arm**, against an **unreproducible bar** | re-derive the frozen-DINOv3 floor, then measure the 4 unmeasured ckpts |
...
| 5 | **drift** | ⛔⛔ **UNVERIFIED-CLAIM** | v7-tiny | drift re-measured with the shuffled-endpoint control subtracted |
| 6 | **decodability** | ⚠️ PARTIAL (L3 **UNDECIDED**) | v7-tiny | re-score `splitp30k` on the corrected L3 instrument (~11 min CPU) |

**`UNVERIFIED-CLAIM` rows: 2 (non-collapse) and 5 (drift).** Both were *marked closed or
