            ...
            for c in clips:
                ...
                with torch.no_grad():
                    ...
                    for i in range(0, len(zt) - W - K, 1):
                        ...
                        # ⭐ THE CONTROL THAT SEPARATES ACTION-CONDITIONING FROM
                        # SMOOTHING. Identical rollout, identical start window, but
                        # the ACTION SEQUENCE is drawn from a RANDOM OTHER TIME in
                        # the same clip. If the true-action rollout still beats
                        # this, the predictor is USING the ego's commanded motion —
                        # an action-conditioned world model, which is the physics
                        # claim. If the two are equal, ẑ's advantage over z_t was
                        # only temporal smoothing and carries no action semantics.
                        win2 = zt[i:i + W][None].to(dev).clone()
                        for s2 in range(K):
                            r0 = int(rng_sh.integers(0, max(1, len(act) - W)))
                            aa2 = act[r0:r0 + W][None].to(dev)
                            if aa2.shape[1] != W:
                                break
                            o2 = w.predictor(win2, torch.cat([aa2, vv], -1))[1]
                            o2 = o2.reshape(1, -1)[:, :zt.shape[1]]
                            win2 = torch.cat([win2[:, 1:], o2[None]], dim=1)
                        zs.append(o2.reshape(-1).cpu().numpy())
                ...
                COL["zhat_t+k (SHUFFLED ACTIONS)"].append(np.stack(zs).astype(np.float64))
                ...
                TG["n_agents"].append(np.array(ya)[:, None])
