...
OUT = SP / "nrmse_shuf_census.json"
# ⛔ THE FULL C149 CENSUS, re-read with the shuffled-action control beside it.
# MODEL_REGISTRY 13.0c now REQUIRES both columns on every row: a
# constant-predictor floor cannot separate temporal extrapolation from
# action-conditioning, and E-DEC-30 showed the programme's three floor-beating
# arms are unchanged to four decimals when their actions are replaced by noise.
ARMS = ['rdw8', 'rdw8b8', 'o5k4', 'o5k8', 'o5k16', 'rdw8o3', 'rdw8o23',
        'splitfrz', 'splitfrz10k', 'frzrand', 'postrain', 'postrain10k',
        'o7w1p0', 'o7w10', 'o7w50', 'o8w1p0', 'o8w10p0', 'o9nb0', 'o9nb4',
        'o1sg', 'o1sgdet', 'o1sgw3', 'psgw003', 'psgw01', 'psgw1', 'psgw3',
        'psgenc01', 'rdw8p30k', 'scale1', 'champ30k', 'splitp30k', 'rdw8s30k']
F = 100
...
def main() -> int:
    ...
    print(f"  {'arm':<13}{'step':>7}{'cos_TRUE':>9}{'cos_SHUF':>11}{'nrmse':>9}"
          f"{'nrmse_SHUF':>11}{'|d-ds|/|d|':>12}{'floor':>10}")
    ...
    for arm in present:
        if not (SP / f"v7tiny_{arm}" / "ckpt.pt").is_file():
            print(f"  {arm:<13}  SKIPPED - no local checkpoint", flush=True)
            continue
        ...
        with torch.no_grad():
            for c in clips:
                ...
                for i in range(0, max(1, len(zt) - W - 2), 5):
                    ...
                    # ⭐ E-DEC-28c: the SAME one-step prediction with the ACTION
                    # SEQUENCE drawn from a random other time. E-DEC-28b showed the
                    # predictor's ENVIRONMENT advantage is action-independent; this
                    # asks the sharper question — is its LATENT-SPACE accuracy
                    # (nrmse 0.7845, the number five arms are ranked on) action-
                    # dependent AT ALL? If nrmse is unchanged under shuffling, the
                    # 'action-conditioned world model' is not conditioned on
                    # actions in any measurable way.
                    r0 = int(rng_sh.integers(0, max(1, len(act) - W)))
                    aa2 = act[r0:r0 + W][None].to(dev)
                    zs_ = w.predictor(win, torch.cat([aa2, vv], -1))[1]
                    zs_ = zs_.reshape(1, -1)[:, :zt.shape[1]]
                    base = zt[i + W - 1].to(dev)
                    D.append((zh.reshape(-1) - base).cpu().numpy())
                    DS.append((zs_.reshape(-1) - base).cpu().numpy())
                    T.append((zt[k].to(dev) - base).cpu().numpy())
        d = np.stack(D).astype(np.float64)
        t = np.stack(T).astype(np.float64)
        ds = np.stack(DS).astype(np.float64)
        ...
        nrmse_shuf = float(np.linalg.norm(ds - t)) / tnorm
        cos_shuf = float((ds * t).sum() / max(np.linalg.norm(ds) * np.linalg.norm(t), 1e-30))
        dd_ = np.linalg.norm(d - ds) / max(np.linalg.norm(d), 1e-30)
        ...
        rep["arms"][arm] = {"step": int(st), "cos_raw": round(cos_raw, 4),
                            ...
                            "nrmse": round(nrmse, 4),
                            "nrmse_SHUFFLED_actions": round(nrmse_shuf, 4),
                            "cos_raw_SHUFFLED": round(cos_shuf, 4),
                            "rel_change_in_prediction_from_shuffling": round(dd_, 4),
                            "nrmse_meanonly_control": round(nrmse_mean, 4),
                            "nrmse_zero_control": 1.0, "n_windows": len(d),
                            "verdict": v}
        print(f"  {arm:<13}{st:>7}{cos_raw:>9.4f}{cos_shuf:>11.4f}{nrmse:>9.4f}"
              f"{nrmse_shuf:>11.4f}{dd_:>12.4f}{nrmse_mean:>10.4f}")
        del w
