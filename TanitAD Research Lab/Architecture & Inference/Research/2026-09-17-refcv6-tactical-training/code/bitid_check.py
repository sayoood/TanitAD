...
tactical weights at their 0.0 default and the same seed:
...
if not (ma.is_file() and mb.is_file()):
    ...
    bad = sorted(k for k in keys
                 if k.startswith(("tacv6", "gp_tac_decoder", "ga_")))
    rep["metrics"]["patched_key_count"] = len(keys)
    rep["metrics"]["tactical_keys_present"] = bad
...
if not (ga.is_file() and gb.is_file()):
    ...
    rep["config"] = {
        ...
        "refcv6_tactical_in_patched": jb.get("refcv6_tactical", "<absent>"),
        "speed_max_derivation_v6_in_patched": jb.get("speed_max_derivation_v6", "<absent>"),
        ...
        "verdict": ("INCONCLUSIVE" if not ja or not jb else
                    ("SAME-KEYS-SAME-VALUES"
                     ...
                     else ("ONLY-refcv6-NULL-KEYS-ADDED"
                           if set(only_b) <= {"refcv6_tactical",
                                              "refcv6_max_speed",
                                              "speed_max_derivation_v6"}
                           and not only_a and not changed
                           and all(jb.get(k) is None for k in only_b)
                           else "DIFFERS")))}
...
# --- 5. CONFIG ADDITIVITY, RECURSIVELY -------------------------------------
# ⛔⛔ WHY A BARE "config DIFFERS" IS NOT AN ANSWER. Adding a flag NECESSARILY
# changes `argv`, `effective_weights` and `seams` — the parser records every
# default, the weight audit is exhaustive, and the seam stamp gains two blocks.
# So the useful question is not *"did config change?"* (it must) but
# **"did any value that EXISTED BEFORE change?"** — because THAT is what would
# alter a run.
#
# ⚠️ The out-dir is excluded by VALUE, not by key: both runs legitimately write
# to different directories, and that string appears inside `argv` and `out`.
# Excluding the KEY `argv` wholesale would hide a real flag-value change, which
# is the one thing this check is for.
def _flat(o, pre=""):
    """{dotted path: leaf value} — so a nested change cannot hide in a dict
    comparison that only looks at the top level."""
    if isinstance(o, dict):
        for k, v in o.items():
            yield from _flat(v, f"{pre}.{k}" if pre else str(k))
    elif isinstance(o, list):
        for i, v in enumerate(o):
            yield from _flat(v, f"{pre}[{i}]")
    else:
        yield pre, o


if (A / "config.json").is_file() and (B / "config.json").is_file():
    fa = dict(_flat(json.loads((A / "config.json").read_text(encoding="utf-8"))))
    fb = dict(_flat(json.loads((B / "config.json").read_text(encoding="utf-8"))))
    ta, tb = str(A).replace("\\", "/"), str(B).replace("\\", "/")

    def _outdirish(x):
        s = str(x).replace("\\", "/")
        return ta in s or tb in s

    changed = sorted(k for k in set(fa) & set(fb)
                     if fa[k] != fb[k] and not (_outdirish(fa[k])
                                                and _outdirish(fb[k])))
    added = sorted(set(fb) - set(fa))
    removed = sorted(set(fa) - set(fb))
    # ⭐ THE SAME-BREATH CONTROL. A comparison that flattened to nothing would
    # report "no pre-existing value changed" with perfect confidence and zero
    # evidence — the empty-string blob-comparison hole in another costume.
    rep["config_additivity"] = {
        "n_leaves_tip": len(fa), "n_leaves_patched": len(fb),
        "n_added": len(added), "n_removed": len(removed),
        "added": added[:40],
        "removed": removed,
        "pre_existing_values_changed": changed,
        "excluded_as_outdir": sorted(
            k for k in set(fa) & set(fb)
            if fa[k] != fb[k] and _outdirish(fa[k]) and _outdirish(fb[k])),
        "verdict": ("INCONCLUSIVE" if min(len(fa), len(fb)) == 0
                    else ("PURELY-ADDITIVE" if not changed and not removed
                          else "CHANGES-A-PRE-EXISTING-VALUE")),
    }
else:
    rep["config_additivity"] = {"verdict": "INCONCLUSIVE",
                                "why": "a config.json is missing"}
