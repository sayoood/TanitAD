...
while true; do
  ...
  pid=$(trainer_pid)
  if [ -z "$pid" ]; then
    # DEFECT FIXED (MEASURED 2026-09-07 on this very run). The inherited guard
    # compared `st` to `prev_st`, but the ALIVE branch below reset `stall=0`
    # and `prev_st=$st` on EVERY poll -- so a trainer that crashes slowly (here:
    # every ~8 min, with ~4 live polls in between) NEVER trips the 3-strike
    # guard. All three relaunches of this run logged `stall=0` while making
    # ZERO net progress (each resumed from ckpt step 2000 and died at 2200).
    # The guard now counts RELAUNCHES THAT DID NOT ADVANCE THE RUN, and only
    # a relaunch can reset it.
    if [ "${st:-0}" -le "${last_relaunch_step:--1}" ]; then
      stall=$((stall + 1))
    else
      stall=0
    fi
    last_relaunch_step=$st
    if [ "$stall" -ge 3 ]; then
    ...
    # ALIVE. ⛔ This branch must NOT touch `stall` -- resetting it here is
    # exactly what defeated the crash-loop guard. A live trainer that makes no
    # progress is a HANG, which is reported LOUDLY and never "fixed" by killing
    # a running job: that decision belongs to a human.
    if [ "${st:-0}" = "${hang_prev:--1}" ]; then
      hang=$((hang + 1))
      if [ "$hang" = 10 ]; then
        log "WARN: trainer $pid ALIVE but step has not advanced past $st for ~$((10 * POLL))s -- possible hang. NOT killing it; escalate to a human."
      fi
    else
      hang=0
      hang_prev=$st
