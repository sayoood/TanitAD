#!/usr/bin/env python3
"""P4a - frozen refcv5-v2 trunk tokens (+ a raw-pixel floor) for every stacked row of the
...
def main() -> int:
    ...
    if args.what == "s32pix":
        ...
        pix = np.lib.format.open_memmap(out / "pix64_u8.npy", mode="w+",
    ...
    tk = np.load(out / ("tokens_s32_fp16.npy" if args.what == "s32pix" else "tokens_s16_fp16.npy"),
                 mmap_mode="r")
    px = np.load(out / "pix64_u8.npy", mmap_mode="r")   # the s32pix run's floor, re-checked
    ...
    checks = {
        ...
        "pix_nonzero_frac": float((px_s > 0).mean()),
        "pix_mean": float(px_s.mean()),
