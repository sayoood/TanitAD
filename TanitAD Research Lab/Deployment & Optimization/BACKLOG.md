...
|---|---|---|---|
| **A1** export guard (**re-scoped 2026-08-03**) | §0.5 | 🔵 next, 0 deps | the fastpath mechanism **did not reproduce at 6 cells** — so the guard's content changes: it must catch **geometry-conditional export failure** (the real defect found) and **wiring-bug-shaped parity errors**, not a fastpath flag. Still keep `set_fastpath_enabled(False)` as cheap insurance |
...
| ✅ **B1** ⭐ **complete decision tick** (absorbed O3 + **O9**) | §0.5 | ✅ **DONE 2026-08-03** — 9 candidates serialised **243.84 ms (244 %)** vs batch-9 engine **56.13 ms (56 %)**. ⇒ **rebuild the engine at batch 9/dynamic — a deployment requirement worth 4.3×.** Corrected-graph engine 1.187 vs published 1.168 ms ⇒ the 5.33× survives |
| 🔴 **O2-pre** (NEW) fix the encoder's shape-derived pooling so it exports at 176×624 | §0.5 | 🔵 **next, 0 deps** | `adaptive_avg_pool2d, output size that are not factor of input size` blocks **O2 entirely**; the 2026-07-08 "encoder exports clean" claim was 256×256 and is false at the shipping geometry. Intake + export test **at the deployed geometry** |
| ✅ **B0** val-window cache on Thor | §0.5 | ✅ **DONE by the orchestrator 2026-08-03** | `~/valdata/physicalai-val-0c5f7dac3b11` (2.4 GB `ep_*.pt`) + `w120-256x640cyl`; `~/models/` carries v1-speedjerk, v5f, refc-*, rollout-recovery. ⚠️ parity-sha vs the committed manifest still to be verified before any arm runs |
| **B2** ⭐ **bf16-vs-fp16 encoder DECISION-agreement** | §3 Tier B | 🔵 **NEXT — unblocked by B0** | the 6.76× lever rests on rel-err 0.0059 on **random weights**, at a precision our own decision-space measurement **rejected** (bf16 → 67.2 % agreement, 47.7 cm). Fails ⇒ O2 becomes P0 and the 51.2 ms is void. ⚠️ its instrument check (Thor fp32 panel must reproduce the banked A40 `fourfam_v1-lf19.json`) runs FIRST |
| **B3** = **O1** four-family gate on the TRT-fp16 engine | §3 Tier B | 🔵 after B2's instrument check | paired episode-cluster bootstrap, per-family, never pooled. ⚠️ gate the **batch-9** engine — that is what ships |
