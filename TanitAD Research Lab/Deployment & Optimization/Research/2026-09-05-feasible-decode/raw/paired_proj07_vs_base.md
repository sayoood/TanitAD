# PAIRED OPEN-LOOP COMPARISON — base vs proj07

`paired_openloop.py` · generated `20260905T173347Z` · **0 GPU** (it reads banked dumps only)

## LINE ONE: beats the trivial floor `ha0` on ADE (paired, separated): **base, proj07**
...
|---|---|---|---|---|
| `base:os` | 0.0000 | 0.0000 | **0.0000** | `proj07:os` 4402/4823 |
| `proj07:os` | 0.0000 | 0.0000 | **0.0000** | `base:os` 4402/4823 |
...
* selection profile `proj07`: **50 distinct anchors**, modal #57 at 0.1482, entropy 2.8425, agrees with the GT-nearest oracle on 0.5652
...
| metric | `base:os` | `proj07:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
...
### LONGITUDINAL — family verdict: **proj07:os**
...
| `TAC_declared_lat:true_minus_navshuffled` | `proj07:os` | 0.0242 | [-0.0017, 0.0544] | no |
...
| `TAC_declared_lat:true_minus_navzero` | `proj07:os` | 0.0294 | [-0.0095, 0.0700] | no |
...
| `TAC_declared_lon:true_minus_navshuffled` | `proj07:os` | 0.0147 | [-0.0052, 0.0354] | no |
...
| `TAC_declared_lon:true_minus_navzero` | `proj07:os` | 0.0182 | [-0.0112, 0.0464] | no |
...
| `STR_route:true_minus_navshuffled` | `proj07:os` | 0.0000 | [0.0000, 0.0000] | no |
...
| `STR_route:true_minus_navzero` | `proj07:os` | 0.0000 | [0.0000, 0.0000] | no |
...
`proj07` publishes: `train_labels_md5` = `0ff902130ce76886b8a925eceed9e3a5`, `train_labels_n_records` = `4572`, `train_labels_path` = `/workspace/TanitAD/data/s2_labels_v7.2_train.jsonl.gz`. Missing: the run's own config.json was not supplied (--a-run-config / --b-run-config).
...
```
python taniteval/tools/paired_openloop.py --a-dump <refav1 FINAL dump> --a-name base --a-arm os --b-dump <refcv3 FINAL dump> --b-name proj07 --b-arm os  --a-run-config <refav1 run config.json> --b-run-config <refcv3 run config.json> --floor ha0 --n-boot 2000 --seed 0 --out taniteval/results/paired-openloop-base-vs-proj07-<UTC>.json --md <RESULT.md>
```
