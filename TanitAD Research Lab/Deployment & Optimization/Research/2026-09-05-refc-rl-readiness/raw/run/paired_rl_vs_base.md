# PAIRED OPEN-LOOP COMPARISON — base vs rl

`paired_openloop.py` · generated `20260905T124034Z` · **0 GPU** (it reads banked dumps only)

## LINE ONE: beats the trivial floor `ha0` on ADE (paired, separated): **base**
...
| arm | `os` | `os` |
| tier | `T1` | `T1` |
| checkpoint | `/workspace/experiments/refcv3-b1-v72-30k/ckpt_40284_FINAL.pt` | `C:/Users/Admin/rl_rescope/run/rl/ckpt/ckpt_after.pt` |
| step | `40284` | `40284` |
...
* selection profile `rl`: **37 distinct anchors**, modal #26 at 0.3054, entropy 2.3241, agrees with the GT-nearest oracle on 0.1702
...
| metric | `base:os` | `rl:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `ade_m` | 0.4419 | 0.9433 | 0.6723 | -0.2304* | 0.2710* | **0.5014** | [0.4572, 0.5464] | **YES** |
| `fde_m` | 0.9288 | 1.3477 | 1.4029 | -0.4741* | -0.0551 | **0.4189** | [0.3530, 0.4870] | **YES** |

### LONGITUDINAL — family verdict: **base:os**

| metric | `base:os` | `rl:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `LON_speed_mae_mps` | 0.4516 | 1.3160 | 0.4880 | -0.0364* | 0.8280* | **0.8643** | [0.7975, 0.9356] | **YES** |
| `LON_along_mae_m` | 0.4030 | 0.8467 | 0.4705 | -0.0674* | 0.3762* | **0.4436** | [0.3993, 0.4906] | **YES** |
| `LON_accel_mae_mps2` | 0.6806 | 3.2793 | 0.4786 | 0.2020* | 2.8007* | **2.5987** | [2.4310, 2.7768] | **YES** |

### LATERAL — family verdict: **base:os**

| metric | `base:os` | `rl:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `LAT_cross_mae_m` | 0.1084 | 0.2795 | 0.3132 | -0.2048* | -0.0337 | **0.1711** | [0.1589, 0.1842] | **YES** |
| `LAT_heading_mae_deg` | 1.3591 | 7.6017 | 2.8715 | -1.3741* | 4.2977* | **5.6507** | [4.4727, 6.9142] | **YES** |
| `LAT_yaw_rate_mae_radps` | 0.2176 | 0.5013 | 0.0476 | 0.1700* | 0.4536* | **0.2836** | [0.2231, 0.3516] | **YES** |

### TACTICAL — family verdict: **base:os**

| metric | `base:os` | `rl:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `TAC_traj_lat_correct` | 0.9540 | 0.7730 | 0.8659 | 0.0881* | -0.0929* | **-0.1810** | [-0.2195, -0.1459] | **YES** |
| `TAC_traj_lon_correct` | 0.7477 | 0.3838 | 0.7576 | -0.0100 | -0.3738* | **-0.3639** | [-0.4015, -0.3278] | **YES** |
...
| `TAC_declared_lat:true_minus_navshuffled` | `rl:os` | 0.0242 | [-0.0017, 0.0544] | no |
...
| `TAC_declared_lat:true_minus_navzero` | `rl:os` | 0.0294 | [-0.0095, 0.0700] | no |
...
| `TAC_declared_lon:true_minus_navshuffled` | `rl:os` | 0.0147 | [-0.0052, 0.0354] | no |
...
| `TAC_declared_lon:true_minus_navzero` | `rl:os` | 0.0182 | [-0.0112, 0.0464] | no |
...
| `STR_route:true_minus_navshuffled` | `rl:os` | 0.0000 | [0.0000, 0.0000] | no |
...
| `STR_route:true_minus_navzero` | `rl:os` | 0.0000 | [0.0000, 0.0000] | no |
...
`rl` publishes: `train_labels_md5` = `0ff902130ce76886b8a925eceed9e3a5`, `train_labels_n_records` = `4572`, `train_labels_path` = `/workspace/TanitAD/data/s2_labels_v7.2_train.jsonl.gz`. Missing: the run's own config.json was not supplied (--a-run-config / --b-run-config).
...
```
python taniteval/tools/paired_openloop.py --a-dump <refav1 FINAL dump> --a-name base --a-arm os --b-dump <refcv3 FINAL dump> --b-name rl --b-arm os  --a-run-config <refav1 run config.json> --b-run-config <refcv3 run config.json> --floor ha0 --n-boot 2000 --seed 0 --out taniteval/results/paired-openloop-base-vs-rl-<UTC>.json --md <RESULT.md>
```
