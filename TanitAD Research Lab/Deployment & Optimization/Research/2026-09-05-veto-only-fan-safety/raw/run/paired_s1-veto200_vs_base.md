# PAIRED OPEN-LOOP COMPARISON — base vs s1-veto200

`paired_openloop.py` · generated `20260905T174626Z` · **0 GPU** (it reads banked dumps only)

## LINE ONE: beats the trivial floor `ha0` on ADE (paired, separated): **base, s1-veto200**
...
| | A | B |
|---|---|---|
| model | `base` | `s1-veto200` |
| arm | `os` | `os` |
| tier | `T1` | `T1` |
| checkpoint | `/workspace/experiments/refcv3-b1-v72-30k/ckpt_40284_FINAL.pt` | `C:/Users/Admin/veto_run/run/s1/veto200/ckpt/ckpt_after.pt` |
| step | `40284` | `40284` |
...
|---|---|---|---|---|
| `base:os` | 0.0000 | 0.0000 | **0.0000** | — |
| `s1-veto200:os` | 0.0000 | 0.0000 | **0.0000** | — |
...
* selection profile `s1-veto200`: **40 distinct anchors**, modal #57 at 0.1644, entropy 2.9303, agrees with the GT-nearest oracle on 0.3386
...
### ADE — family verdict: **base:os**

| metric | `base:os` | `s1-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `ade_m` | 0.4419 | 0.4895 | 0.6723 | -0.2304* | -0.1828* | **0.0475** | [0.0369, 0.0587] | **YES** |
| `fde_m` | 0.9288 | 0.9965 | 1.4029 | -0.4741* | -0.4064* | **0.0677** | [0.0448, 0.0907] | **YES** |

### LONGITUDINAL — family verdict: **base:os**

| metric | `base:os` | `s1-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `LON_speed_mae_mps` | 0.4516 | 0.5144 | 0.4880 | -0.0364* | 0.0265 | **0.0628** | [0.0468, 0.0800] | **YES** |
| `LON_along_mae_m` | 0.4030 | 0.4342 | 0.4705 | -0.0674* | -0.0363* | **0.0312** | [0.0203, 0.0425] | **YES** |
| `LON_accel_mae_mps2` | 0.6806 | 0.8963 | 0.4786 | 0.2020* | 0.4177* | **0.2157** | [0.1614, 0.2768] | **YES** |

### LATERAL — family verdict: **base:os**

| metric | `base:os` | `s1-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `LAT_cross_mae_m` | 0.1084 | 0.1406 | 0.3132 | -0.2048* | -0.1726* | **0.0322** | [0.0278, 0.0368] | **YES** |
| `LAT_heading_mae_deg` | 1.3591 | 1.8715 | 2.8715 | -1.3741* | -0.9234* | **0.4517** | [0.3911, 0.5193] | **YES** |
| `LAT_yaw_rate_mae_radps` | 0.2176 | 0.2654 | 0.0476 | 0.1700* | 0.2178* | **0.0478** | [0.0355, 0.0612] | **YES** |

### TACTICAL — family verdict: **base:os**

| metric | `base:os` | `s1-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `TAC_traj_lat_correct` | 0.9540 | 0.9401 | 0.8659 | 0.0881* | 0.0742* | **-0.0139** | [-0.0205, -0.0071] | **YES** |
| `TAC_traj_lon_correct` | 0.7477 | 0.6826 | 0.7576 | -0.0100 | -0.0751* | **-0.0651** | [-0.0869, -0.0447] | **YES** |
...
| `TAC_declared_lat:true_minus_navshuffled` | `s1-veto200:os` | 0.0242 | [-0.0017, 0.0544] | no |
...
| `TAC_declared_lat:true_minus_navzero` | `s1-veto200:os` | 0.0294 | [-0.0095, 0.0700] | no |
...
| `TAC_declared_lon:true_minus_navshuffled` | `s1-veto200:os` | 0.0147 | [-0.0052, 0.0354] | no |
...
| `TAC_declared_lon:true_minus_navzero` | `s1-veto200:os` | 0.0182 | [-0.0112, 0.0464] | no |
...
| `STR_route:true_minus_navshuffled` | `s1-veto200:os` | 0.0000 | [0.0000, 0.0000] | no |
...
| `STR_route:true_minus_navzero` | `s1-veto200:os` | 0.0000 | [0.0000, 0.0000] | no |
...
`s1-veto200` publishes: `train_labels_md5` = `0ff902130ce76886b8a925eceed9e3a5`, `train_labels_n_records` = `4572`, `train_labels_path` = `/workspace/TanitAD/data/s2_labels_v7.2_train.jsonl.gz`. Missing: the run's own config.json was not supplied (--a-run-config / --b-run-config).
...
```
python taniteval/tools/paired_openloop.py --a-dump <refav1 FINAL dump> --a-name base --a-arm os --b-dump <refcv3 FINAL dump> --b-name s1-veto200 --b-arm os  --a-run-config <refav1 run config.json> --b-run-config <refcv3 run config.json> --floor ha0 --n-boot 2000 --seed 0 --out taniteval/results/paired-openloop-base-vs-s1-veto200-<UTC>.json --md <RESULT.md>
```
