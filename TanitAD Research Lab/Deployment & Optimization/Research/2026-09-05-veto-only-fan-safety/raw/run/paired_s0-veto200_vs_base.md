# PAIRED OPEN-LOOP COMPARISON — base vs s0-veto200

`paired_openloop.py` · generated `20260905T160746Z` · **0 GPU** (it reads banked dumps only)

## LINE ONE: beats the trivial floor `ha0` on ADE (paired, separated): **base, s0-veto200**
...
| | A | B |
|---|---|---|
| model | `base` | `s0-veto200` |
| arm | `os` | `os` |
| tier | `T1` | `T1` |
| checkpoint | `/workspace/experiments/refcv3-b1-v72-30k/ckpt_40284_FINAL.pt` | `C:/Users/Admin/veto_run/run/s0/veto200/ckpt/ckpt_after.pt` |
| step | `40284` | `40284` |
...
| `s0-veto200:os` | 0.0000 | 0.0000 | **0.0000** | — |
...
* selection profile `s0-veto200`: **41 distinct anchors**, modal #57 at 0.1650, entropy 2.9307, agrees with the GT-nearest oracle on 0.3400
...
| metric | `base:os` | `s0-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `ade_m` | 0.4419 | 0.4781 | 0.6723 | -0.2304* | -0.1942* | **0.0362** | [0.0261, 0.0460] | **YES** |
| `fde_m` | 0.9288 | 0.9910 | 1.4029 | -0.4741* | -0.4118* | **0.0623** | [0.0401, 0.0837] | **YES** |

### LONGITUDINAL — family verdict: **base:os**

| metric | `base:os` | `s0-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `LON_speed_mae_mps` | 0.4516 | 0.5295 | 0.4880 | -0.0364* | 0.0415* | **0.0779** | [0.0616, 0.0955] | **YES** |
| `LON_along_mae_m` | 0.4030 | 0.4321 | 0.4705 | -0.0674* | -0.0384* | **0.0290** | [0.0188, 0.0392] | **YES** |
| `LON_accel_mae_mps2` | 0.6806 | 1.0155 | 0.4786 | 0.2020* | 0.5369* | **0.3349** | [0.2719, 0.4052] | **YES** |

### LATERAL — family verdict: **base:os**

| metric | `base:os` | `s0-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `LAT_cross_mae_m` | 0.1084 | 0.1238 | 0.3132 | -0.2048* | -0.1894* | **0.0154** | [0.0111, 0.0200] | **YES** |
| `LAT_heading_mae_deg` | 1.3591 | 1.6542 | 2.8715 | -1.3741* | -1.0829* | **0.2912** | [0.2312, 0.3591] | **YES** |
| `LAT_yaw_rate_mae_radps` | 0.2176 | 0.2367 | 0.0476 | 0.1700* | 0.1891* | **0.0191** | [0.0060, 0.0316] | **YES** |

### TACTICAL — family verdict: **base:os**

| metric | `base:os` | `s0-veto200:os` | `shared:ha0` | A−floor | B−floor | (B−f)−(A−f) | 95% CI | separated |
|---|---|---|---|---|---|---|---|---|
| `TAC_traj_lat_correct` | 0.9540 | 0.9515 | 0.8659 | 0.0881* | 0.0856* | **-0.0025** | [-0.0066, 0.0015] | no |
| `TAC_traj_lon_correct` | 0.7477 | 0.6587 | 0.7576 | -0.0100 | -0.0989* | **-0.0889** | [-0.1145, -0.0655] | **YES** |
...
| `TAC_declared_lat:true_minus_navshuffled` | `s0-veto200:os` | 0.0242 | [-0.0017, 0.0544] | no |
...
| `TAC_declared_lat:true_minus_navzero` | `s0-veto200:os` | 0.0294 | [-0.0095, 0.0700] | no |
...
| `TAC_declared_lon:true_minus_navshuffled` | `s0-veto200:os` | 0.0147 | [-0.0052, 0.0354] | no |
...
| `TAC_declared_lon:true_minus_navzero` | `s0-veto200:os` | 0.0182 | [-0.0112, 0.0464] | no |
...
| `STR_route:true_minus_navshuffled` | `s0-veto200:os` | 0.0000 | [0.0000, 0.0000] | no |
...
| `STR_route:true_minus_navzero` | `s0-veto200:os` | 0.0000 | [0.0000, 0.0000] | no |
...
`s0-veto200` publishes: `train_labels_md5` = `0ff902130ce76886b8a925eceed9e3a5`, `train_labels_n_records` = `4572`, `train_labels_path` = `/workspace/TanitAD/data/s2_labels_v7.2_train.jsonl.gz`. Missing: the run's own config.json was not supplied (--a-run-config / --b-run-config).
...
```
python taniteval/tools/paired_openloop.py --a-dump <refav1 FINAL dump> --a-name base --a-arm os --b-dump <refcv3 FINAL dump> --b-name s0-veto200 --b-arm os  --a-run-config <refav1 run config.json> --b-run-config <refcv3 run config.json> --floor ha0 --n-boot 2000 --seed 0 --out taniteval/results/paired-openloop-base-vs-s0-veto200-<UTC>.json --md <RESULT.md>
```
