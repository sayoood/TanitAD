"""E-DEP-VGEO-3 — path-length value geometry + matched-pixel distractors (see ../SPEC.md, hash in ../raw/prereg_pin.json).

CPU only, below-normal priority. Writes raw/vgeo3_pathlen.json.
"""
...
STEP, NBOOT, SEED = 4, 2000, 20260913
...
vshift = {}
for c in keep_clips:
    T = len(speed[c]); off = int(rng.integers(T // 3, 2 * T // 3 + 1))
    vshift[c] = np.concatenate([[0.0], np.cumsum(np.roll(speed[c], off) * 0.1)])
...
for c in sc_clips:
    ...
    other = sc_rows[rclip[sc_rows] != c]
    pool = rng.choice(other, min(3000, len(other)), replace=False)
    dtrue = np.linalg.norm(Xp[i] - Xp[j], axis=1)
    dpool = np.linalg.norm(Xp[i][:, None, :] - Xp[pool][None, :, :], axis=2)   # [200, 3000]
    dd = pool[np.argmin(np.abs(dpool - dtrue[:, None]), axis=1)]
    trip[int(c)] = (i, j, dd)
...
for name, F in feats.items():
    ...
    if name == "latent_7040":
        ...
        arms["C_vshift|latent_7040_diag"] = {"P1": p1(diag, "vshift")[0]}
        arms["C_mut|latent_7040_diag"] = {"P1": p1(diag, "mut")[0]}
    print(name, "done", round(time.time() - t0, 1), flush=True)

res = {"spec": "SPEC.md E-DEP-VGEO-3", "bank": BANK, "speed_source": GT, "clips_kept": int(len(keep_clips)),
       ...
       "fit_meta": meta, "content_checks": content, "n_triplet_clips": len(trip),
