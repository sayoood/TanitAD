#!/usr/bin/env bash
# ============================================================================
# flagship v4.1 30k launch - pod2 (tanitad-pod2), 2026-07-22.
# FIX vs v4: --lr-trunk 3e-5 (10x lower). v4's WM canary degraded 0.42->1.3 by
#   step 3500, driven by the trunk LP-FT at lr_trunk=3e-4 (kept climbing after the
#   controller zeroed the planner gradient => trunk FT, not planner). No trunk-freeze
#   knob exists in the trainer (trunk is one AdamW group at cosine lr_trunk from step 0),
#   so the fix is the global lr reduction.
# Warm-start: the V1 trunk flagship4b-speedjerk-30k (NOT the degraded v4 ckpt).
#   Fresh --out => no resume => clean warm-start from --trunk.
...
PY=/usr/bin/python3
OUT=/workspace/experiments/flagship-v4.1-30k
...
ARGS=(
  ...
  --batch 16 --lr-head 1e-4 --lr-trunk 3e-5 --eval-every 500 --save-every 1000
