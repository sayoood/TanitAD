...
A_KEY, B_KEY = "v4fs-mixed40", "v4fs-produced40"   # delta = mixed(oracle speed) - produced
...
print(f"{'metric':12s} {"mixed":>9s} {"produced":>9s} {'delta':>10s} {'CI95':>22s}  sep")
rows = {}
for name, a, b in tests:
    ...
    rows[name] = {"mixed": float(a.mean()), "produced": float(b.mean()),
...
json.dump({"estimator": "paired episode-cluster bootstrap (taniteval.ci), B=2000, seed=0",
           "delta_sign": "mixed(oracle speed) - produced", "n_windows": len(eid),
           "n_episodes": len(set(eid)), "verdict": verdict, "rows": rows},
          open(f"{OUT}/paired_ceiling.json", "w"), indent=1)
print(f"-> {OUT}/paired_ceiling.json")
