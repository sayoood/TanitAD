...
>
> This is the **Opponent Analyzer** agent's findings log. The router across all areas is
...
- [2026-09-02] [PUBLISHED-PRIMARY lib `2605.09701` (repaired), corroborated by lib `2606.07170`] **CW-1 RESOLVES FROM THE PAPER'S OWN TEXT: "All ablations are conducted WITHOUT GTRS-Dense scorer"** - DriveFuture's 30.9-34.6 ablation rows and its 55.5 navhard headline are different configurations, named as such; TOAD independently reproduces DriveFuture at 55.5 in its own six-planner table. ⭐ The same caption makes the ablation an ISOLATION of the world model: future-aware conditioning is worth **+3.7 EPDMS (30.9 -> 34.6)**, while the remaining +20.9 to the headline arrives with the dense scorer.
  impact: **recommend NARROWING the fence to "admissible with its setting" (PI/MM call, not the Lab's); and the strongest published LATENT-WORLD-MODEL navhard result is ~85% scorer, ~15% world model - a PUBLISHED datapoint for our own selection-surface finding** - `TanitAD Research Lab/Opponent Analysis/Research/2026-09-02-cw1-resolution-and-evidence-integrity/RESULT.md`
- [2026-09-02] [MEASURED ours, byte-level check over all 310 banked entries] **`kb_add.py --verify` GAVE THE SAME VERDICT FOR A CORRUPT LIBRARY AND A HEALTHY ONE** - `2605.09701` was banked at 1,373,148 B against a true 9,453,407 B (**14.5%**, no `%%EOF` anywhere, unparseable) while `--verify` reported "310 entries, 0 orphan(s), **0 problem(s)**" - before AND after the repair, because it re-hashes against the hash recorded at bank time. 309 of 310 entries are fine; the one failure was the exact paper an open correction-watch needed.
  impact: **`--verify` proves a file has not CHANGED, never that it was COMPLETE - backlog row 36's hardening must add a COMPLETENESS check at bank time (`%%EOF` + a real parse). Repaired this pass and re-verified against the correct hash** - same package
- [2026-09-02] [PUBLISHED-PRIMARY lib `2606.07170` Tables 3/6/7] **`LEADERBOARD.md:1036`'s "DrivoR 56.3 EPDMS navhard, arXiv 2606.07170" IS WRONG ON TWO COUNTS** - 56.3 is **DrivoR + TOAD** (test-time CEM search on a frozen planner); **bare DrivoR is 54.6** (confirmed by two independent primaries); and `2606.07170` is **TOAD's** id, DrivoR is `2601.05083`. The correct setting was already banked in the Library's own note ("w/ TTO") and dropped downstream.
  impact: **same error class as CW-1 (a level quoted across settings), sitting one line below CW-1's own subject and unflagged, in a block declared a merge target for the empty NavSim table; backlog row 32's efficiency wedge names the wrong leader - DriveFuture 55.5 is the strongest LEARNED method and bare DrivoR is 4th** - same package

- [2026-09-05] [PUBLISHED-BLOG wayve.ai/thinking/gaia-4, 2026-08-03] First Wayve adjudication (register V-1..V-4): GAIA-4 closed loop is EGO-ONLY against a frozen world by their own concession — "no vehicle, pedestrian, or cyclist changes its behavior in response to the AI Driver" — and the validity claim rests on unnumbered "early studies" while `2403.16092` says the real-to-sim gap must itself be MEASURED — impact: our binding open/closed-loop ruling SURVIVES and GAIA-4 sits on our side of it; backlog row 14 (NuRec/gsplat pseudo-simulation) is our GAIA-4 and is unranked and unexecuted — link: `Frontier Scan/Daily/2026-09-05/RESULT.md` F1
- [2026-09-10] [NVIDIA Technical Blog, Ivanovic & Pavone 2026-05-31, PUBLISHED-BLOG] NVIDIA's closed-loop post-training doctrine post reports **zero metrics and zero ablations** — every number is an operational spec — while Sim2Real-AD `2604.03497` and AD-R1 (CVPR 2026) independently contest the mechanism (*"direct deployment fails even when the policy performs well in simulation"*) — impact: **register rows A16-1 CONFIRMS-US (the diagnosis) / A16-2 and A16-3 CONTESTED (the prescription); it binds on us as diagnosis, not prescription; new debt D-12 — no AlpaSim fidelity characterisation exists, and we run AlpaSim so it is answerable by measurement** — `Opponent Analysis/OPPONENT_CLAIMS_REGISTER.md` rows A16-1…A16-3
- [2026-09-10] [arXiv 2609.00111v1 Qwen-Drive-1.0-4B, full text, Apache 2.0] A 5.0 B VLM-native driving foundation model (Qwen3.5-4B + BEV perception head + flow-matching Planning Expert, four training stages) that **wins every open-loop board it reports and finishes near the bottom of its own closed-loop table**, concedes that **multi-timescale causal structure is unsolved** (*"the model remains unstable in identifying the governing cause and its temporal scope"*) and that its reasoning gain **may be a conditioning-context artifact** — impact: **the strongest external evidence for S-1; independent confirmation of the three-planner hierarchy directive as an open problem; a good perception teacher and labeller for us but a poor planning-distillation teacher, since its trajectory head is exactly where it is weakest and its candidate diversity is measurably narrow (avg-minADE gap 0.03 vs Alpamayo's 0.19)** — `Opponent Analysis/Research/2026-09-10-qwen-drive-teardown/RESULT.md`

- [2026-09-17] [PUBLISHED-BLOG waymo.com 2026-02 + PUBLISHED-CARD nvidia/Alpamayo2-Super] **Debt D-1 DISCHARGED after 17 days**: Waymo's world model is **simulation-only, Genie 3-based, with NO controlled experiment of any kind** — its two concessions are ours (*long simulations lose stable quality*; *reconstructive sim breaks under missing observations*, a direct hazard to backlog row 14); and **NVIDIA moved the ENTIRE Alpamayo lineup to OpenMDW-1.1** (commercial redistribution, 34 B = 32 B backbone + 2.3 B action expert, AlpaSim closed-loop 1.50 +/- 0.13) — impact: guideline **S-5** (*"world model" names two different artifacts and evidence about one is not evidence about the other*), and **register row N-3's licence restriction may be SUPERSEDED** — `TanitAD Research Lab/Frontier Scan/Daily/2026-09-17/RESULT.md` §1

- [2026-09-18] [Tesla FSD v14 Lite release notes (PUBLISHED-RELEASE-NOTE) + Elluswamy talks (RELAYED)] **Tesla's distillation doctrine is SUPPORTED as practice but its RL-transfer claim is unmeasured, and its own notes concede the distilled student ships supervised; Elluswamy's "open-loop loss might not correlate" CONFIRMS our tier doctrine** — T-18-1…3, S-7, T-8, T-9; register D-2/D-3 bookkeeping corrected — `Frontier Scan/Daily/2026-09-18/RESULT.md`
- [2026-09-19] [NVIDIA AI Ascent talk RELAYED + NVIDIA Tech Blog 2026-06-15; Waabi blog 2025-03-11; Mobileye blog 2024-05-15] Seven opponent claims adjudicated: "VLAs are dead" CONTESTED (NVIDIA's own Alpamayo 2 is a 34 B VLA; NVIDIA concedes 3–4× WAM latency), pixel-coupling UNSUPPORTED-AS-STATED (two controlled target ablations), Waabi 99.7 % realism UNSUPPORTED-AS-STATED as a safety standard (an average hides the tail), Mobileye bias-variance SUPPORTED as sample complexity with a concession that E2E vs modular is not the real axis — S-8 / T-10…T-12 — `Frontier Scan/Daily/2026-09-19/RESULT.md` §1
- [2026-09-20] [1604.06915 full text + Waymo ReD blog] Two oldest Band-D debts closed. Mobileye's "exponentially larger" is a CONSTRUCTION conditional on an approximately-independent conjunctive factorisation -- so it argues FOR a structured design and hands H1b a precise burden. Waymo's ReD is NOT-ADJUDICABLE (experiment paywalled), but NIEON's own concession is that it "inherits the pre-conflict behaviors ... of the ADS being evaluated" -- our T1 self-action open loop, in an opponent's safety case. — impact: H1b re-framed and re-priced; no ReD number quotable — [Opponent Analysis/Research/2026-09-20-red-and-the-modularity-proof/RESULT.md]
