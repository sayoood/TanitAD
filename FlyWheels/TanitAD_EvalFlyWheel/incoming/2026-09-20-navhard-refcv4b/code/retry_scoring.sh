...
say "retry v2 armed: reuse-seams $SEAMS · reuse-floors $FLOORS · require $REQUIRE"
...
while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
  ...
  wait_for_headroom
  OUT="$PKG/raw/retry_v2_attempt${attempt}.log"
  ...
  ( cd "$REPO/taniteval" && env PYTHONIOENCODING=utf-8 OMP_NUM_THREADS=16 \
      ...
      --reuse-seams "$SEAMS" --reuse-floors "$FLOORS" > "$OUT" 2>&1 )
  ...
  if [ -n "$RD" ]; then
    "$PY" "$PKG/code/judge_run.py" "$RD" --require "$REQUIRE" > "$PKG/raw/retry_v2_attempt${attempt}.judge.txt" 2>&1
    J=$(grep '^JUDGE=' "$PKG/raw/retry_v2_attempt${attempt}.judge.txt" | tail -1)
    say "attempt $attempt: $J — $(head -1 "$PKG/raw/retry_v2_attempt${attempt}.judge.txt" | cut -c1-240)"
