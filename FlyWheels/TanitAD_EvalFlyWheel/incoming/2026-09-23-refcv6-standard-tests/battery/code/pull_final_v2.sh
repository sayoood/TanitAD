#!/bin/sh
# SPEC A5 (v2, for chain_final_v2.sh): READ-ONLY pull of the FINAL refcv6 checkpoint (Thor run dir ckpt.pt),
# plus the run's config.json (the post-switch argv), only after summary.json says
...
i=0
while [ $i -lt 300 ]; do
  # read-only scp (the brief allows scp and ssh -n ls/md5sum/grep/stat only): no file yet = not done
  rm -f "$RAW/summary_poll.json"
  timeout 120 scp -q -o BatchMode=yes $H:$RUN/summary.json "$RAW/summary_poll.json" 2>/dev/null
  d=$(cat "$RAW/summary_poll.json" 2>/dev/null)
...
CFG="$RAW/config_final_${TS}.json"
timeout 300 scp -q -o BatchMode=yes $H:$RUN/config.json "$CFG" || { echo "ZZFINALFAILZZ config-scp"; exit 3; }
cm=$(md5sum "$CFG" | cut -d' ' -f1)
[ ${#cm} -eq 32 ] || { echo "ZZFINALFAILZZ config-md5-unreadable"; exit 3; }
echo "ZZFINALOKZZ md5 $m3 step $st metrics metrics_${TS}.jsonl config config_final_${TS}.json cfgmd5 $cm"
