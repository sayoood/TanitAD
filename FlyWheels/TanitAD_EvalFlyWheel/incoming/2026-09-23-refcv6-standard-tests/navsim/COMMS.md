...
| D21 | the FINAL is rebuilt with the kit's `config.json`, not the resumed run's: the resumed config (scp'd read-only 2026-09-26) differs only by `--clip-clock-sidecar` (training data) and the `data_order` / `label_clock` records; the fe5872f eval stack's trainer parser may not know the new flag, and no model-construction flag changed. `load_refcv6`'s STRICT load is the RED check | a rebuild from a config the eval stack cannot parse would fail; a silent architecture change would fail strict load |
| D22 | the model-as-trained stamp is STEP-AWARE (`code/model_stamp6.py`, 5 literal tests incl. an off-by-one mutation that goes RED): ≤ 34,500 "F3 detach-only, F4 on the last layer only; tactical labels ~0.37 s early", after it "hybrid: F3 cascade loss + true label clock from step 34,500"; written into every summary / BARS; `step_compare.py` flags any pre- vs post-switch comparison. Step-5,000 and 30,000 summaries re-stamped by RE-PARSING (`code/restamp6.py` proves nothing else changed) | Master Mind 2026-09-26 (PI: "Stop now, resume with fixes" at 34,500) |
| D23 | the two EXPENSIVE splits (navtest full, navhard) wait up to 3 h for the GPU gate (env `R6_EXPENSIVE_GPU_WAIT_S`) before a CPU pass; the cheap splits keep 15 min | MEASURED 2026-09-26: at 97 % box load a CPU bridge ran 4.6 s/scene ⇒ ~15 h / ~16 h on CPU vs ~1 h / ~2.5 h on the card |
| D24 | every milestone after 5,000 writes `compare_vs_step5000_<split>.json` (`code/step_compare.py`: paired on identical tokens; warmup S2-EPDMS-u vs both seed floors; navhard / navtest paired log-cluster bootstrap; flags a device mismatch and a fix-switch straddle); identity smoke test reads 0.0 with a [0, 0] interval on all three splits | the step-to-step read is the mid-run question |
