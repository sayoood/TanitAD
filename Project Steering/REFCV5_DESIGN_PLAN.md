...
| **11** | ⚠️ **(B)'s own manifest says it registered ELEVEN `D-V5A-*` rows; the register holds TEN** | — | (B)'s status line and §5.1: *"11 `D-V5A-*` + 8 `H-V5A-*` rows registered"* | MEASURED 2026-09-05 by **two independent probes** on `GOALS_AND_CLAIMS.md` (854,085 B, read OK): an id regex over the whole file, **and** a direct read of the `## D-V5A / H-V5A` block at lines 2723–2746. Both return **10 `D-V5A-*`** (`CAM1`, `CAM2`, `CAM3`, `GROUND1`, `THORSHARE`, `ATT2OF3`, `OCC-LIMIT`, `CYLINDRICAL`, `LADDER`, `BOUNDARY`) and **8 `H-V5A-*`** — the H count matches, the D count is one short. ⇒ **either a row did not land or (B)'s count is off by one.** Per (A) §10's own rule the **register is the source**; the Master Mind decides which end to fix. ⛔ **I did not add a row to make the count match** |
