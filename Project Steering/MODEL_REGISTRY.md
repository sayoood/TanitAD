...
### 4.6 REF-C **v4b** — `refcv4b-b1-v72-40k` — 🟢 **TRAINING** (launched 2026-09-04 11:40:23 UTC) · ⛔ **NO RESULT YET — no checkpoint has been evaluated; the only number in this row is a MODEL-FREE vocabulary ceiling and may never be read as driving performance**
...
| **Status** | 🟢 **TRAINING** — ⚠️ **no LANDING result; a PRELIMINARY zero-GPU read of `ckpt_30000.pt` now exists and is NOT a result** (`TanitAD Research Lab/Architecture & Inference/Research/2026-09-06-refcv4b-landing/PRELIM_CPU_PANEL.md`): 171 windows / 20 episodes, CPU, against refcv3 @40,284 on the SAME surface with the model-free arms bit-identical across both runs — `os` **0.3055 vs 0.4707 m**, paired **−0.1652 [−0.2318, −0.1090] separated**, the gain concentrated in LONGITUDINAL decision κ (0.2037 → 0.5782). ⛔ It still LOSES to `ha` 0.2860 and `ha0_ext` 0.2769, and `--ablate ego_zero` reads **1.1137 m** (κₓ 0.5782 → 0.0653) so the win is NOT vision-only. ⛔ 20 clips, one seed, five levers — an ARM delta on a subset, never a lever attribution and never a registry-grade level. The FIRST model-inclusive gate, answerable from a pulled checkpoint on the same 4,823 windows: `oracle_sel` ≥ 0.2996 separated ⇒ kill (`D-REFCV4-GATE1` §1.4); the four-family open-loop suite (`taniteval/tools/openloop_suite.py`, criteria registry) at the end. **Do not quote `anchor_acc`, `goal_str` or any `metrics.jsonl` value as a result — T0 loss-surface diagnostics, C1** |
