...
| N5 | Architecture | **REF-B first full training launch** (rev2: strategic transformer d384×4 + route-nav commands, budget-matched 260.7 M, built+merged) — blocked on pod3 comma-extraction completing. ⏹ **CLOSED 2026-08-16** — blocker CLEARED. REF-B launched and ran to completion through four versions. Evidence (MEASURED): `Project Steering/MODEL_REGISTRY.md:1928` §3.5 — `refb-refbpatch-v2-30k` (271,619,880 params) ADE@2s **0.5921 ± 0.0685 @ step 29999**, marked ✅ FINAL. Swept by the 2026-08-16 stale-blocker sweep. | `REFARCH` REF-B section; `PS` §4 ("training GO now only awaits Sayed's rev2 look"); `RESOURCE_LEDGER.md` row `refb-post30k` ($40 confirmed in chat) | D4 arbitration: does imagine-and-select beat a matched-budget hierarchical BC stack? Falsifier: REF-B matches us on D8/LOPS/SC-01 (structurally shouldn't) → WM premise wounded | ~~Prototype (code built/tested; 0 training steps run at scale yet)~~ → ⏹ **DELIVERED** — 30k steps run, see CLOSED note in the Deliverable cell | Pod1/pod3, ~2 days, $40 envelope (pre-confirmed) |
...
| N7 | Tools&DevEnv | **TanitScena v1** — parse `SCENARIO_DATABASE.md` (14 entries) into a structured store + local vector index (semantic search) + TanitResim-language UI. | D-029 (2); `tools-devenv-agent.md` P2 | Top-1 retrieval correct on ≥8/10 held-out natural-language queries; falsifier: worse than keyword grep | ~~**Prototype, pre-code** (`tanitscena` branch touched only `DECISIONS.md`+agent docs; `stack/tanitad/scena/` does not exist yet)~~ → ⏹ **CLOSED 2026-08-16** — blocker CLEARED on **2026-07-12** (commit `9ebfb09`, "scena: TanitScena scenario-database app (parser, local vector search, visual UI)"), i.e. the same day this row was written. `stack/tanitad/scena/` **exists in HEAD**: `parse.py` (SCENARIO_DATABASE.md→structured store), `vector.py` (local MiniLM/hashing-TF-IDF semantic index), `static/` (app.js + index.html + style.css UI), `__init__.py`, `README.md`; plus `stack/scripts/scena_app.py` and tests `stack/tests/test_scena.py`. Evidence (MEASURED): `git ls-tree HEAD -- stack/tanitad/scena/`. Swept by the 2026-08-16 stale-blocker sweep. | Local, days, $0 |
| N8 | Benchmarks&Eval | **nuScenes-mini bench harness** — loader + metric trajectory head + community L2@{1,2,3}s + collision rate, reported WITHOUT the AD-MLP ego-status shortcut. | `DIAG` §2 ("Build: nuScenes-mini loader... Phase-0.5 build"); `LB` open-loop table (TanitAD row "—", "Phase 1 target: first entry"); D-002 (nuScenes-mini named for OOD probes) | First honestly-placed community-standard number with the shortcut caveat attached; no pass/fail yet — this is placement, not a claim | ~~**Prototype, not started** (no nuScenes loader exists in `stack/` yet)~~ → ⏹ **CLOSED 2026-08-16** — the *loader* half is CLEARED: `stack/tanitad/data/nuscenes.py`, `stack/scripts/ingest_nuscenes.py` and `stack/tests/test_nuscenes.py` all exist in HEAD (two probes: `git ls-files`, and a `nuscenes` grep across `stack/`). ⚠️ **The loader existing is NOT the benchmark row** — the L2@{1,2,3}s + collision-rate placement is still unclaimed, and the nuScenes Terms remain a PI-side item (`BOOST_PROGRAM.md:288`). Evidence (MEASURED): `stack/tanitad/data/nuscenes.py`. Swept by the 2026-08-16 stale-blocker sweep. | Local/4060, days, $0 |
...
> ⏹ **PARTIALLY CLOSED / SUPERSEDED 2026-08-16** — P1.3's stated blocker ("blocked on a
> graphics-capable pod") rests on a **RETRACTED premise**. *"Our pods cannot render / no EGL
> devices"* was retracted **2026-07-21** as a C2 single-probe absence: the Vulkan ICD is in
> `/etc/vulkan/icd.d/`, not `/usr/share/` — `Project Steering/RETRACTION_LOG.md:35`, which records
> that the false absence **"blocked AlpaSim + CARLA for 12 days"**.
...
> still reads *"CARLA pixels remain host-blocked"*, and the root cause recorded in §5 below is a
> **different** mechanism from the retracted ICD-path claim (RunPod compute-only driver caps + a
> UE4.24 Vulkan-offscreen bug). No CARLA-pixels run was found. ⇒ **CARLA-pixels: still blocked
> (UNVERIFIABLE as cleared); the "no graphics-capable pod" reason: refuted; the closed-loop goal:
> met elsewhere.** Swept by the 2026-08-16 stale-blocker sweep.
...
> ⏹ **CLOSED 2026-08-16** — blocker CLEARED on **2026-07-12** (commit `9ebfb09`). `stack/tanitad/scena/`
> was implemented the same day this paragraph was written: `parse.py`, `vector.py`, `static/` UI,
> `stack/scripts/scena_app.py`, tests `stack/tests/test_scena.py`. "Start the implementation" is **not**
> this discipline's gap-closer any more; the open question is whether it meets N7's own falsifier
> (top-1 correct on ≥8/10 held-out NL queries vs keyword grep), which has **not** been measured.
> Evidence (MEASURED): `git ls-tree HEAD -- stack/tanitad/scena/`. Swept by the 2026-08-16 stale-blocker sweep.
...
| **TanitScena** | ~~Prototype, pre-code~~ → ⏹ **Built** | ~~D-029 mandate + agent-file scaffolding only (`0ab6060`); `stack/tanitad/scena/` does not exist; the `tanitscena` branch has touched only docs~~ ⏹ **CLOSED 2026-08-16** — CLEARED **2026-07-12** by commit `9ebfb09`; `stack/tanitad/scena/{parse,vector,__init__}.py` + `static/` UI + `stack/scripts/scena_app.py` + `stack/tests/test_scena.py` are in HEAD. Evidence (MEASURED): `git ls-tree HEAD -- stack/tanitad/scena/`. Swept by the 2026-08-16 stale-blocker sweep. | ~~Build the markdown→structured-store parser…~~ → the parser is built; the **unmeasured** item is N7's falsifier (top-1 ≥8/10 held-out NL queries vs keyword grep) |
