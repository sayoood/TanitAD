...
| 6 | How is the ResNet trained and used, vs DiffusionDrive and other AD works? | **DD:** ImageNet ResNet-34 (21.8 M) fine-tuned end-to-end at half the learning rate, with perception losses, 100 epochs; **frozen in V2**. **Ours:** a random-init 90.5 M trunk, **one epoch of 25.5 h**, no perception loss. Every compared work pretrains its trunk. Our frozen trunk is ≈ raw pixels on a BEV probe. Measured since: its tokens **do** carry their position; training concentrated its features ≈ 4× relative to random init; and **4.2× more training exposure did not separably increase its scene content** (+0.011 AP, CI covers 0). ⇒ the lever is the **supervision or the prior**, not exposure. §6 |
...
| 7 | ⛔ **Training exposure is not the lever either:** refcv4b at 9,500 vs 40,284 steps of the *same* run reads AP 0.3972 → 0.4080, **+0.0108 [−0.0078, +0.0309], not separated** and below 3× the seed floor. Neither checkpoint separates from raw pixels | `D-REFCV4B-EXPOSURE-1`, `…/2026-09-15-refcv6-clarification/raw/h4_panel.json` (MEASURED 2026-09-16) |
...
| H4 | ~~The trunk is data-limited~~ **REFUTED as pre-registered (MEASURED 2026-09-16)** | Same frozen-trunk BEV probe, same rows, same split, **only the checkpoint differs** (md5-identical run configs). refcv4b **@40,284 AP 0.4080** vs **@9,500 AP 0.3972**: **+0.0108, CI [−0.0309, +0.0078], not separated**, and below 3× the probe-seed floor (0.0045 → bar 0.0135). Gates passed: constant reads the prevalence 0.2761858 **exactly**, zero-information 0.3801 ≤ prior + 0.02. ⚠️ 4.2× more exposure on **one** run; exposure is confounded with optimisation progress, and a probe bounds what a *probe* recovers. ⇒ points away from exposure and **toward H2** (planning-only losses do not ground a trunk) | done (2.5 GPU-h) | `…/raw/h4_panel.json`, `h4_prereg.md` |
...
⭐ **Default:**
1. H3, H5 (CPU) and **H4 (2.5 GPU-h, overnight)** have run. H4 is refuted, which **raises the weight on H1 and H2**: if neither exposure nor scale moves the trunk's scene content, the lever is the **supervision**, or the **prior**.
