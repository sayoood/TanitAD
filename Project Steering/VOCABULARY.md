...
| **nav command (v7.2)** | ⭐ **GROUND TRUTH and a FIRST-CLASS INPUT** (PI ruling 2026-09-04). It is a ROUTE signal — the thing a map router supplies at deployment ("turn left at the next junction") — **not** an ego-future leak. It was validated and there is no better source. ⛔ Do NOT stamp it "oracle", do NOT call it "absent at deployment", and do NOT treat `os_navzero` as the deployment arm: **`os` (nav fed) is the deployment-relevant arm; `os_navzero` is a ROBUSTNESS ABLATION.** ⚠️ One true nuance to state when comparing to published work: because ours is derived from the ego's own future path, it is NOISELESS and perfectly timed, where a real router is coarser — so it is a CLEAN version of a deployment signal, not an absent one. | PI ruling 2026-09-04 |
| **ego-future leakage** | the thing that IS inadmissible: the car's own future trajectory, future speeds, future actions or future poses from GT at inference. ⚠️ Distinct from a route command — conflating the two is C18's error | 2026-09-04 |
