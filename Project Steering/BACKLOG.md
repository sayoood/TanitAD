...
| A12 | **Ship the 4 stale/absent files to Thor** — `sel_winners_curse_law.py` (absent), `lf0_bev_lead.py`, `p8_bev_reel.py`, `refc_train.py` | found only after the closure audit's entry points were **derived** (161 files from 52 entries, vs 134 from the old hand-list of 14). **Not on the S-T launch path** ⇒ ship with the next legitimate sync, **not mid-run**: `launch_closure_audit.py --ship --backup-dir …` |
...
| A15 | ⛔ **Adjudicate `…/incoming/2026-08-02-thor-deployment-profile/thor_profile.py`** — the banked script **cannot have produced its co-banked `thor_profile.json`** (the JSON carries `"frame": "176x624 hfov 117.0"`; the banked script never assigns `out['frame']`) | **MEASURED 2026-08-18.** The producer was Thor-only and is now at `…/2026-08-18-thor-stranded-rescue/rescued/home_nvidia_root/thor_profile.py` (+15 lines: `resolve_v2_frames`, `speed_input=True`, `action_dim=3` — i.e. **a different model was profiled**). ⚠️ `pod_git_drift.py` saw this file and classified it **`NAME_ONLY` = "weak evidence, not drift"** ⇒ raise `NAME_ONLY` severity for program-authored basenames, and widen `SUFFIXES` beyond `.py`/`.sh` (`:91`) — it missed **54 %** of the stranded set |
