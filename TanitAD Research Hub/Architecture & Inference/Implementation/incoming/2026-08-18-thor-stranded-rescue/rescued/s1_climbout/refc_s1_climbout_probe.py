...
def loeo_scores(feats: dict, names: list[str], keep: torch.Tensor,
                de_all: torch.Tensor, eid) -> tuple[torch.Tensor, dict]:
    ...
    for e in uniq:
        ...
        assert not bool((torch.as_tensor(te) & torch.as_tensor(tr)).any())
        w = _fit(phi[tr], tgt[tr], keep[tr])
        out[te] = (phi[te] * w).sum(-1)
        ws.append(w.numpy())
    w_full = _fit(phi, tgt, keep)                    # for the C-leak in-sample row
    return out, {"n_folds": len(uniq), "w_mean": np.mean(ws, 0).tolist(),
...
def run(bank: str, arm: str, out_dir: str) -> dict:
    ...
    pk = torch.gather(keep, 1, perm)
    psc, _ = loeo_scores(pf, FEATURE_SETS["D-lon+scores"], pk, de_all, eid)
    pidx = argmax_over_survivors(psc, pk)
    ...
    for name, (idx, blk) in arms.items():
        fams[name] = {
            ...
            "absolute": P.families_block(d, idx, eid, sel_half=blk, tag=name),
        }
...
def _sep_dir(p: dict, better_is_negative: bool = True) -> tuple[bool, bool]:
    ...
    """
    lo, hi = p.get("ci_lo"), p.get("ci_hi")
    mean = p.get("mean", p.get("delta"))
    sep = bool(p.get("separated", (lo is not None and hi is not None
                                   and (lo > 0 or hi < 0))))
