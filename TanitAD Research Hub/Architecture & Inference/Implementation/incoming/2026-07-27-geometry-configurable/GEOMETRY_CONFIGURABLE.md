...
| **8** | ⛔ **A LIVE DEFECT IN TODAY'S TRAINING INPUT, reproduced: the deployed crop pads 11.2 % of rows on rig B and 0 % on rig A** — ~29 % of the corpus carries fabricated rows that correlate perfectly with rig. **A wide *crop* inherits it (and 120° nearly doubles it to 19.9 %); the cylindrical path removes the fabrication entirely** (mask, not replicate-pad) and cuts rig B's asymmetry to **0.69 % at 100°**. | **MEASURED** |
...
Rig A (cy ≈ 543) spills nothing. So **~29 % of the corpus carries fabricated rows that correlate
