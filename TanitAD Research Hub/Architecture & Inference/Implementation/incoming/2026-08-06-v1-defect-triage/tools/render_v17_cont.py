...
def bev(size,gt,b,fonts):
    ...
    for i,(lab,col) in enumerate((("ground truth",COL_GT),("v1.7 (speed-profile readout)",COL_B))):
...
def main():
    ...
    fr=os.path.join(os.path.dirname(a.out) or ".","_v17c"); shutil.rmtree(fr,ignore_errors=True); os.makedirs(fr)
    ...
    for fi,(epf,dpf) in enumerate(zip(files,dumps)):
        z=np.load(dpf); B,G,ws=z["o6"],z["g"],z["ws"]
        ...
        for wi in range(0,len(ws),a.frame_stride):
            ...
            d.text((PAD,4),f"flagship v1.7 (flagship-v17-speedloss) — CONTINUOUS, real-time 10 fps · "
