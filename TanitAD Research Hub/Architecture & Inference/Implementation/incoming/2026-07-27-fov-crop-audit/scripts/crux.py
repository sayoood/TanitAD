...
⚠️ RESCUED INTO GIT 2026-07-27 by the fov-crop-audit stream. This file previously existed ONLY in a
dev-box scratch directory while THREE streams depended on it for their headline numbers — the H2
sensor-attention study, the situation classifier's per-camera need table, and this FOV audit. Under
`Project Steering/AGENT_OPERATING_STANDARD.md` rule 1 that is exactly the failure the standard
exists to prevent, so it is staged here verbatim (only this docstring added).

It is staged in THIS folder because that is where it can be staged without touching a
concurrently-edited `stack/`. **It belongs in `stack/tanitad/data/` and that move is escalation 4 of
`FOV_CROP_AUDIT.md` — a decision for the owner of the geometry layer, not for this stream.**

Provenance: authored by the H2 stream; unmodified apart from this header.
"""
