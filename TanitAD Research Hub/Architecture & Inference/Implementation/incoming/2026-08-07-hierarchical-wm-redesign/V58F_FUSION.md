...
| W7 = X0 full | WM-roll MPC re-rank top-8 (needs W3 pass) | 5 | 2 h | ≥50 % sel_gap closed at T1 — **RUN ×4 2026-08-10/11: FAIL on both trunks, two DIFFERENT mechanisms.** Frozen trunk K-sweep (k8/k32/k64): roll-cost is the programme's first calibrated selector (ρ 0.399 at K=8) but muffled rolls (W3 gain 0.27) cannot separate similar candidates — collapses at dense K. **Repaired (stage-A) trunk K=32: selected 2.347 vs thr 0.4505 (frac closed −2.27) — but the in-run FROZEN selector reads 3.448 (banked 0.7933) and fan oracle 0.289 (banked 0.1077): the stage-A repair moved the trunk feature distribution and the W4 emission head + frozen selector (both trained on FROZEN-trunk features, `w4_meta.w4_base_ckpt`) no longer compose — an INSTRUMENT-COMPOSITION failure, not a repair verdict.** Signal FOR the repair inside the same artifact: across-window roll-cost calibration ρ **0.7164 [0.5847, 0.7696]** (P7 gate PASS; ~1.8× the frozen trunk's 0.399), and W7 beats the in-run frozen pick on 71.1 % of windows. ⇒ **W4r = refit the (cheap, 4000-step) unicycle emission head on the stage-A trunk, then re-run W7** — queued behind P8 on pod5. Artifact `w7_gate_repaired_k32.json`; registry §1.14. |
