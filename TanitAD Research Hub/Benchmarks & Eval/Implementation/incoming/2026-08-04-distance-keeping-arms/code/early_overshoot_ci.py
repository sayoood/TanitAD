"""EARLY OVERSHOOT: the arm is already past the lead's rear face at the FIRST waypoint (0.5 s).

⚠️ The bare 'gap < 0 somewhere' indicator is NOT a clean contact precursor — GT and the CV floor
both trip it, with overruns up to ~39 m, because the ego legitimately passes a stopped or
turning-off lead. What the human's own path does ZERO times in 270 windows is be past the lead
already at 0.5 s. That is the discriminating form, so that is the one with a CI on it.
"""
import json, sys
...
for a, P in arms.items():
    ...
    for j, i in enumerate(idx):
        ...
        v[j] = float(np.isfinite(gap[0]) and gap[0] < 0)     # already past at 0.5 s
    ind[a] = v
out = {'_what': 'EARLY OVERSHOOT rate: predicted path already past the lead rear face at t+0.5 s',
       '_denominators': {'lead_windows': int(idx.size), 'all_windows': int(ST.size)},
       '_estimator': 'episode-cluster bootstrap, paired for deltas, B=2000 seed 0',
       'rate': {}, 'paired': {}}
for a in arms:
    r = episode_cluster_bootstrap(ind[a], eids, n_boot=2000, seed=0)
    out['rate'][a] = {'n': int(ind[a].sum()), 'over_lead_windows': round(float(ind[a].mean()), 4), **r}
    print(f"{a:<16} {int(ind[a].sum()):>3}/270  rate {r['mean']:.4f} [{r['lo']:.4f}, {r['hi']:.4f}]")
