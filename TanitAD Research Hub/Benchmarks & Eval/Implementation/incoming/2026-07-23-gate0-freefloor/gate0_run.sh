#!/bin/bash
# gate0_run.sh -- run REF-C-base + the free floor over the 38-scene suite in a clean
# logdir. Usage: bash gate0_run.sh <logdir> <on|off> <floor_log> <sceneset_dir>
# Renderer MUST be up on :6011 (shared). Runtime FOREGROUND; kills own controller/
# driver/physics by port (6006/6007/6789, never 6011). Writes <logdir>/DONE.
set -uo pipefail
ALPA=/workspace/alpa-invest/alpasim
PY="$ALPA/.venv/bin/python"
LOGDIR="$1"; FLOOR="$2"; FLOORLOG="$3"; SSDIR="$4"
TAG=$(basename "$LOGDIR")
...
grep -q 'allow_aggregation_with_failed_rollouts: true' "$LOGDIR/eval-config.yaml" && log "eval: allow_aggregation=true SET"
...
DRVA="--port 6789 --ckpt /root/models/refc-base-30k/ckpt.pt --preset base --floor $FLOOR --lam 5.0 --mu 0.0 --clamp-m 0.75 --sceneset-dir $SSDIR --floor-log $FLOORLOG"
setsid env PYTHONPATH=/root/TanitAD/stack:/root/TanitAD/stack/scripts:/workspace \
  "$PY" /workspace/refc_floor_driver.py $DRVA \
...
if [ "$UP" -lt 4 ]; then
  ...
  tail -30 /workspace/${TAG}_driver.log | sed 's/^/DRIVER: /' | tee -a "$LOG"
...
NFLIP=$(grep -oP '"base_idx": \K[0-9]+' "$FLOORLOG" 2>/dev/null | wc -l)
log "aggregate=$([ -f "$AGG" ] && echo present || echo MISSING) n_rollout_id=$NROLL canon=$FEFF clamp_fires=$NCLAMP floor_drives=$NFLIP"
