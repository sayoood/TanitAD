...
| **Baseline adequacy in the safety case** (credible testing) | a capability claim is evidence only against a **representative** null — an inadequate comparator overstates demonstrated capability | trivial-floor family in `taniteval/driving.py` + the paired episode-cluster separation predicate | **MEASURED 2026-08-02:** the family was `(cv, holdv0)` — **both straight lines** — so lateral/turn capability was scored against a null that cannot turn. Adding CTRV moves **16 of 25** arms' verdicts and shrinks the flagship's turn margin **5.3×** (still separated). `…/incoming/2026-08-02-ctrv-floor/` + LEADERBOARD §1b | **gap found, fix proposed** (patch + 11 tests, awaiting triage) |
