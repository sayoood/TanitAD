...
from tanitad.data import tactical_goals as TG         # noqa: E402
...
def main() -> None:
    ...
    for p in sorted(EPC):
        ...
        # ⭐ TACTICAL GOALS — derived over the 2-6 s band. Previously ABSENT
        # from the pipeline entirely, which is why the report had none to show.
        gtac = TG.derive(poses, key=KEY, hz=HZ,
                         stop_reason=(sem.stop_reason if sem else None))
        stops = EM.stop_episodes(poses[KEY:, 3])
        prop_tok, prop_why = TG.strategic_from_geometry(
            man, stop_onset_s=(stops[0][0] / HZ if stops else None),
            turn_onset_s=man.yaw_onset_s)
        ...
        out.append({
            ...
            "tactical": tactical,
            "g_tac": gtac.as_dict(),
            "geom_strategic": {"token": prop_tok, "why": prop_why},
    ...
    (OUT / "sample_v4.json").write_text(json.dumps(out), encoding="utf-8")
    slim = [{k: v for k, v in r.items() if k != "frames"} for r in out]
    (OUT / "sample_v4_slim.json").write_text(json.dumps(slim, indent=1),
                                             encoding="utf-8")
    n = len(out)
    print(f"[v4] {n} clips")
