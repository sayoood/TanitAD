...
def score_arm(*, arm: str, prof: "P.SplitProfile", raw_dir: Path, exp_dir: Path, seam: Path | None = None,
              ...
              python: Path = P.PY, log=print, baseline_blobs: dict | None = None) -> dict:
    ...
    ov = [f"train_test_split={prof.name}", f"experiment_name={exp_name}",
    ...
    cmd = [str(python), str(P.WRAPPER), "--script", "pdm_score", "--label", arm, "--out-dir", str(raw_dir),
    ...
    expected = prof.n_stage1 + prof.n_stage2
    fails = []
    ...
    preagg_rows = None
    if preagg.exists():
        with open(preagg, encoding="utf-8") as fh:
            preagg_rows = max(sum(1 for _ in fh) - 1, 0)
