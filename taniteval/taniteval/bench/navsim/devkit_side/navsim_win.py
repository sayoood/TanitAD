...
def main() -> int:
    ...
    # --- W1 ADDITION (2026-09-20) -------------------------------------------------------------
    ap.add_argument("--dump-preaggregation", type=Path, default=None,
                    help="W1 ADDITION: dump the per-token rows the PARENT receives from worker_map "
                         "BEFORE any aggregation, so a crash in create_scene_aggregators / "
                         "compute_final_scores / calculate_individual_mapping_scores cannot destroy a "
                         "completed scoring run (MEASURED 2026-09-19: navhard CV scored all 5,912 "
                         "scenarios in 68 min, died in aggregation, wrote NO CSV, and — because the run "
                         "used a PROCESS POOL, whose children do not inherit the pdm_score hook — the "
                         "wrapper recorded pdm_calls=0. worker_map returns in the PARENT for every "
                         "worker type, so this seam is pool-safe.)")
    # --- end W1 ADDITION ----------------------------------------------------------------------
    ...
    # --- W1 ADDITION (2026-09-20): PRE-AGGREGATION dump at the worker_map seam ----------------
    if a.dump_preaggregation is not None and hasattr(mod, "worker_map"):
        _orig_wm = mod.worker_map

        def worker_map_hook(worker, fn, data_points):
            rows = _orig_wm(worker, fn, data_points)
            try:
                import pandas as _pd
                df = _pd.concat(rows)
                a.dump_preaggregation.parent.mkdir(parents=True, exist_ok=True)
                df.to_csv(a.dump_preaggregation, index=False)
                # the CSV is the readable record; the PICKLE keeps the array columns
                # (ego_simulated_states) that the two-frame comfort needs, so the run can be
                # RE-AGGREGATED with the devkit's own functions (reaggregate.py).
                pkl = a.dump_preaggregation.with_suffix(".pkl")
                df.to_pickle(pkl)
                manifest["preaggregation_dump"] = {"path": str(a.dump_preaggregation), "pickle": str(pkl),
                                                   "n_rows": int(len(df)),
                                                   "columns": [str(c) for c in df.columns]}
                write_all("RUNNING")     # persist NOW: the aggregation may die after this returns
                print(f"E1_WRAPPER_PREAGG rows={len(df)} -> {a.dump_preaggregation}", flush=True)
            except Exception as e:                                   # noqa: BLE001
                manifest["preaggregation_dump_error"] = repr(e)
            return rows

        mod.worker_map = worker_map_hook
        manifest["hooks"].append(f"{SCRIPTS[a.script]}.worker_map (W1: pre-aggregation dump)")
    # --- end W1 ADDITION ----------------------------------------------------------------------
