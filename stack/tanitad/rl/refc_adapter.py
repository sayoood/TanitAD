...
def _missing_channels(required, batch) -> list[str]:
    ...
    MEASURED cost of the whole per-batch check: 0.181-0.184 us over two runs, against
    a 435.3 ms deployed-scale forward and a 3.2 ms smoke forward — see :class:`ConditioningContract`.
    """
...
class ConditioningContract:
    ...
    * `refc.py:1722` — ``if v_ms is None ...: v = full(ref_speed)``
    ...
    ⛔ AND THE COST WAS MEASURED, NOT ASSERTED. CPU-only, this box, TWO independent
