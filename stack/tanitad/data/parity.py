...
def check_uids(uids: Sequence[str], *, corpus_key: str, label: str,
    ...
    if len(obs) < n_exp:
        print(f"[parity] ⚠ {label}: {corpus_key} SUBSET — {len(obs)} of {n_exp} "
              f"episodes ({n_exp - len(obs)} absent). This is admissible ONLY as a "
              f"deliberate episode-subset run; it is NOT strict parity and must not "
              f"be cross-compared with full-corpus arms.", flush=True)
    else:
