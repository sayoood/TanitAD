...
def main() -> int:
    ...
    # GATED, matching the program's existing tanitad-* repos and the standing
    # authorization's own words ("back the final ckpt up to gated HF").
    #
    # A first attempt used private=True and died on
    #   "Private repository storage limit reached"
    # — the account's private quota is full; the gated-public repos are what fit.
    #
    # ORDER MATTERS: create PRIVATE and empty, turn gating ON, only then make it
    # public, and upload last. That way the weights never exist in a public
    # ungated state even briefly.
    api.create_repo(REPO, repo_type="model", private=True, exist_ok=True)
    api.update_repo_settings(REPO, repo_type="model", gated="manual")
    api.update_repo_settings(REPO, repo_type="model", private=False)
    print(f"repo ready (PUBLIC + GATED=manual, still empty): {REPO}", flush=True)
...
Gated repository (public listing, manual access approval) — the program pattern.
"""
