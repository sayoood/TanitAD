...
OUT=/workspace/rrft
S=/workspace/rr20.status
echo "=== RR20 START $(date -u) ===" >> "$S"
...
/workspace/venv/bin/python -u scripts/train_flagship4b.py \
  ...
  --sigreg-free-dims 64 --rollout-k 20 --steps 32000 \
  --batch-size 8 --accum 8 --grad-checkpoint --lr 3e-4 --warmup 0 \
  --ckpt-every 500 --log-every 25 --workers 4 --speed-input \
  --out "$OUT" >> /workspace/rr20.log 2>> /workspace/rr20.err
rc=$?
echo "=== RR20 EXIT $(date -u) rc=$rc ===" >> "$S"
tail -20 /workspace/rr20.err >> "$S"
