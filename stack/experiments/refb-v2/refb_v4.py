...
class TacticalHead(nn.Module):
    ...
    def __init__(self, cfg: TacticalHeadConfig, state_dim: int, window: int,
        ...
        if anchored:
            ...
            self.maneuver_to_anchor = nn.Linear(cfg.n_maneuvers, anchor_n,
                                                bias=False)
            # H26 fix (2026-07-18): a ZERO-INIT learnable gate on the H19
            # maneuver prior. The prior starts as a NO-OP and ramps up only as
            # far as training finds useful, so it can NEVER structurally swamp
            # the decoder's own anchor confidences. (The earlier LayerNorm
            # pinned the prior at a fixed norm sqrt(N)=~11.3 and swamped conf as
            # conf_norm decayed 21.9->5.7 by step 100 -> mode collapse to 1.)
            self.anchor_prior_gate = nn.Parameter(torch.zeros(1))
        else:
    ...
    def forward(self, states: Tensor, ctx: Tensor):
        ...
        if self.anchored:
            ...
            # H19 maneuver prior through a ZERO-INIT gate (starts a no-op).
            prior = self.anchor_prior_gate * self.maneuver_to_anchor(
                torch.log_softmax(man_logits, dim=-1))
            anchor_logits = dec["anchor_logits"] + prior      # H19 (gated)
            ...
            extra = {"anchor_logits": anchor_logits,
                     ...
                     "_dbg_prior_gate":
                         self.anchor_prior_gate.detach().abs().mean()}
        else:
