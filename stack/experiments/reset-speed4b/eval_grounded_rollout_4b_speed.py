...
K_MAX = max(WP_STEPS)
SPEED_SCALE = 10.0        # MUST match flagship_losses: v0 = pose_last[:,3] / 10.0
...
def collect(world, step_readout, episodes, corpora, device, window, fwd_k,
    ...
    channel, EXACTLY as flagship_losses does in training."""
...
def main():
    ...
    ap.add_argument("--speed-input", action="store_true",
                    help="model was trained with v0 as 3rd action channel "
                         "(action_dim=3); append it at eval too")
