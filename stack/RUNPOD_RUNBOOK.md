...
> ## ⛔ HISTORICAL — DO NOT RUN THIS. (banner added 2026-08-16)
> This runbook trains the **261 M 4-brain** stack on **RunPod**, and neither exists in the fleet any
> more: the live model is **v6 (336.5 M)** via `scripts/train_v6_staged.py`, and it runs on **Thor**.
> All four pods were released 2026-08-15. Kept for provenance and for the doctrine in §§5–8.
> ⇒ **The current operator runbook is `…/incoming/2026-08-07-hierarchical-wm-redesign/
> V6_GO_PACKAGE.md` §2**, whose commands are generated by `scripts/v6_chain.py` and pinned by
...
Re-run the script (or `tail -f` the log) to watch again. ⛔ **To stop training, kill by EXPLICIT
PID.** *(Corrected 2026-08-16: this line used to say `pkill -f train_worldmodel`, which is the
self-matching trap two other documents in this repo explicitly ban — the pattern matches your own
ssh command line, kills your session, and returns empty output so it looks like nothing happened.)*
...
```bash
tail -f /workspace/experiments/p0-sB01-realmix.log
```
*(Corrected 2026-08-16: this said `p0-sB01.log`, a file nothing ever writes — §6 and
`pod_launch.sh:11` both write `p0-sB01-realmix.log`. An empty `tail` at 3 a.m. reads as "the run is
dead", which is the most expensive possible way for a typo to fail.)*
